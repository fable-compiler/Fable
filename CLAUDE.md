...
**Test suites** are in `tests/<target>/` (e.g., `tests/Python/`, `tests/Js/Main/`). Tests are first run on .NET, then transpiled to `temp/tests/<target>/` (e.g., `temp/tests/py/`, `temp/tests/js/`) and executed with the target's test runner. Full test runs take several minutes. You can and should run `./build.sh test <target>` yourself after making changes — use `--skip-fable-library` when only compiler code changed to speed things up.
