...
module PY =
    ...
    let [<Global>] NaN: float = pyNative
    let [<Global>] Infinity: float = pyNative
    let [<Global>] Math: Math = pyNative
    let [<Global>] JSON: JSON = pyNative
    let [<Global>] eval: string -> string = pyNative
    let [<Global>] isFinite: float -> bool = pyNative
    let [<Global>] isNaN: float -> bool = pyNative
    let [<Global>] parseFloat: string -> float = pyNative
    let [<Global>] parseInt: string -> int -> int = pyNative
    let [<Global>] decodeURI: string -> string = pyNative
    let [<Global>] decodeURIComponent: string -> string = pyNative
    let [<Global>] encodeURI: string -> string = pyNative
    let [<Global>] encodeURIComponent: string -> string = pyNative
    let [<Global>] console : Console = pyNative
    let [<Global>] setTimeout (callback: unit -> unit) (ms: int): int = pyNative
    let [<Global>] clearTimeout (token: int): unit = pyNative
    let [<Global>] setInterval(callback: unit -> unit) (ms: int) : int = pyNative
    let [<Global>] clearInterval (token: int): unit = pyNative
    let [<Emit("debugger")>] debugger () : unit = pyNative
    let [<Emit("void 0")>] undefined<'a> : 'a = pyNative
    ...
    let [<Global>] Number: NumberConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Object: ObjectConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Date: DateConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Map: MapConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] WeakMap: WeakMapConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Set: SetConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] WeakSet: WeakSetConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Promise: PromiseConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] RegExp: RegExpConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Array: ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] DataView: DataViewConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] ArrayBuffer: ArrayBufferConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] ArrayBufferView: ArrayBufferViewConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Int8Array: Int8ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Uint8Array: Uint8ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Uint8ClampedArray: Uint8ClampedArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Int16Array: Int16ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Uint16Array: Uint16ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Int32Array: Int32ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Uint32Array: Uint32ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Float32Array: Float32ArrayConstructor = pyNative

    [<Obsolete(CONSTRUCTORS_WARNING)>]
    let [<Global>] Float64Array: Float64ArrayConstructor = pyNative

    // [<ObsoleCONSTRUCTORS_WARNING)>]
    // let [<Global>] BigInt64Array: BigInt64ArrayConstructor = pyNative
    ...
    module Constructors =
        ...
        let [<Global>] Float64Array: Float64ArrayConstructor = pyNative
        let [<Global>] BigInt64Array: BigInt64ArrayConstructor = pyNative
        // let [<Global>] BigUint64Array: BigUint64ArrayConstructor = pyNative
