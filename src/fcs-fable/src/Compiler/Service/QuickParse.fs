...
type PartialLongName =
    ...
    static member Empty(endColumn: int) =
        {
            QualifyingIdents = []
            PartialIdent = ""
            EndColumn = endColumn
...
module QuickParse =
    ...
    let GetPartialLongName (lineStr: string, index: int) =
        if isNull lineStr then
            ...
            let IsLongIdentifierPartCharacter pos =
                IsLongIdentifierPartCharacter lineStr[pos]
            ...
            let rec InLeadingIdentifier (pos, right, (prior, residue)) =
                ...
                if pos < 0 then
                    ...
                    InLeadingIdentifier(pos - 1, right, (prior, residue))
                elif IsDot pos then
                    InLeadingIdentifier(pos - 1, pos, PushName())
                else
                    PushName()
            ...
            let rec InName (pos, startResidue, right) =
                ...
                if pos < 0 then
                    ...
                    InLeadingIdentifier(pos - 1, pos, NameAndResidue())
                else
                    NameAndResidue()

            let rec InResidue (pos, right) =
                if pos < 0 then
                    [], lineStr.Substring(pos + 1, right - pos)
                elif IsDot pos then
                    InName(pos - 1, pos, right)
                elif IsLongIdentifierPartCharacter pos then
                    InResidue(pos - 1, right)
                else
                    [], lineStr.Substring(pos + 1, right - pos)
    ...
    let GetPartialLongNameEx (lineStr: string, index: int) : PartialLongName =
        if isNull lineStr then
            ...
            let IsEndOfComment pos =
                pos < index - 1 && lineStr[pos] = '*' && lineStr[pos + 1] = ')'

            let IsStartOfComment pos =
                pos < index - 1 && lineStr[pos] = '(' && lineStr[pos + 1] = '*'
            ...
            let rec SkipWhitespaceBeforeDotIdentifier (pos, ident, current, throwAwayNext, lastDotPos) =
                if pos > index then
                    PartialLongName.Empty(index) // we're in whitespace after an identifier, if this is where the cursor is, there is no PLID here
                elif IsWhitespace pos then
                    SkipWhitespaceBeforeDotIdentifier(pos + 1, ident, current, throwAwayNext, lastDotPos)
                elif IsDot pos then
                    AtStartOfIdentifier(pos + 1, ident :: current, throwAwayNext, Some pos)
                elif IsStartOfComment pos then
                    EatComment(1, pos + 1, EatCommentCallContext.SkipWhiteSpaces(ident, current, throwAwayNext), lastDotPos)
                else
                    AtStartOfIdentifier(pos, [], false, None) // Throw away what we have and start over.

            and EatComment (nesting, pos, callContext, lastDotPos) =
                if pos > index then
                    PartialLongName.Empty(index)
                else if IsStartOfComment pos then
                    // track balance of closing '*)'
                    EatComment(nesting + 1, pos + 2, callContext, lastDotPos)
                else if IsEndOfComment pos then
                    if nesting = 1 then
            ...
            and InUnquotedIdentifier (left: int, pos: int, current, throwAwayNext, lastDotPos) =
                ...
                else if IsIdentifierPartCharacter pos then
                    InUnquotedIdentifier(left, pos + 1, current, throwAwayNext, lastDotPos)
                elif IsDot pos then
            ...
            and InQuotedIdentifier (left: int, pos: int, current, throwAwayNext, lastDotPos) =
                if pos > index then
                    ...
                    if IsTick pos && remainingLength > 1 && IsTick(pos + 1) then
                        ...
                        InQuotedIdentifier(left, pos + 1, current, throwAwayNext, lastDotPos)
    ...
    let TestMemberOrOverrideDeclaration (tokens: FSharpTokenInfo[]) =
        ...
        if filteredReversed.Length < expected.Length then
            false
        else
            // check whether sequences match
            (filteredReversed, expected)
            ||> Seq.forall2 (fun tok expect -> expect |> Array.exists (TokenNameEquals tok))
