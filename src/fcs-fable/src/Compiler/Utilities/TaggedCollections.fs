...
module SetTree =
    ...
    let inline private asNode (value: SetTree<'T>) : SetTreeNode<'T> = value :?> SetTreeNode<'T>
    ...
    let rec intersectionAux comparer b (t: SetTree<'T>) acc =
        if isEmpty t then
            ...
            | :? SetTreeNode<'T> as tn ->
                ...
                let acc =
                    if contains comparer tn.Key b then
            ...
            | _ ->
                if contains comparer t.Key b then
    ...
    type SetIterator<'T>(s: SetTree<'T>) =
        ...
        let notStarted () =
            raise (System.InvalidOperationException("Enumeration has not started. Call MoveNext."))

        let alreadyFinished () =
            raise (System.InvalidOperationException("Enumeration already finished."))
...
type internal Set<'T, 'ComparerTag> when 'ComparerTag :> IComparer<'T>(comparer: IComparer<'T>, tree: SetTree<'T>) =
    ...
    member s.Remove(x) : Set<'T, 'ComparerTag> =
        refresh s (SetTree.remove comparer x tree)
    ...
    interface System.Collections.IEnumerable with
        override s.GetEnumerator() =
            (SetTree.toSeq tree :> System.Collections.IEnumerator)

    static member Singleton(comparer, x) : Set<'T, 'ComparerTag> = Set<_, _>.Empty(comparer).Add(x)
...
module MapTree =
    ...
    let inline private asNode (value: MapTree<'Key, 'Value>) : MapTreeNode<'Key, 'Value> = value :?> MapTreeNode<'Key, 'Value>
...
type internal Map<'Key, 'T, 'ComparerTag> when 'ComparerTag :> IComparer<'Key>(comparer: IComparer<'Key>, tree: MapTree<'Key, 'T>) =
    ...
    static member Empty(comparer: 'ComparerTag) =
        Map<'Key, 'T, 'ComparerTag>(comparer = comparer, tree = MapTree.empty)
    ...
    member m.Filter(f) =
        MapTree.filter comparer f tree |> refresh m
    ...
    member m.Remove(k) =
        refresh m (MapTree.remove comparer k tree)
