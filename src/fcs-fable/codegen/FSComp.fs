...
type internal SR private() =
    ...
    static member ValueNotContainedMutabilityTypesDifferNullness(a0 : System.String, a1 : System.String, a2 : System.String) = (sprintf "Nullness warning: Module '%s' contains\n    %s    \nbut its signature specifies\n    %s    \nThe types differ in their nullness annotations" a0 a1 a2)
    ...
    static member FieldNotContainedTypesDifferNullness(a0 : System.String, a1 : System.String) = (sprintf "Nullness warning: The module contains the field\n    %s    \nbut its signature specifies\n    %s    \nThe types differ in their nullness annotations" a0 a1)
    ...
    static member csComparisonDelegateConstraintInconsistent() = (474, sprintf "The constraints 'comparison' and 'delegate' are inconsistent" )
    static member csNullStructConstraintInconsistent() = (475, sprintf "The constraints 'struct' and 'null' are inconsistent" )
    ...
    static member optsTypecheckOnly() = (sprintf "Perform type checking only, do not execute code" )
    ...
    static member lexfltInvalidNestedTypeDefinition() = (sprintf "Nested type definitions are not allowed. Types must be defined at module or namespace level." )
    static member lexfltInvalidNestedModule() = (sprintf "Modules cannot be nested inside types. Define modules at module or namespace level." )
    static member lexfltInvalidNestedExceptionDefinition() = (sprintf "Exceptions must be defined at module level, not inside types." )
    static member lexfltInvalidNestedOpenDeclaration() = (sprintf "'open' declarations must appear at module level, not inside types." )
    static member lexfltInvalidNestedConstruct(a0 : System.String) = (sprintf "'%s' must be defined at module level, not inside a type." a0)
    ...
    static member tcPropertyCannotBeSetPrivateSetter(a0 : System.String) = (3248, sprintf "Property '%s' cannot be set because the setter is private" a0)
    ...
    static member tcUnitToObjSubsumption() = (3397, sprintf "This expression uses 'unit' for an 'obj'-typed argument. This will lead to passing 'null' at runtime. This warning may be disabled using '#nowarn \"3397\"." )
    ...
    static member tcExpectedTypeParamMarkedWithUnitOfMeasureAttribute() = (3874, sprintf "Expected unit-of-measure type parameter must be marked with the [<Measure>] attribute." )
    ...
    static member featureSupportWarnWhenUnitPassedToObjArg() = (sprintf "Warn when unit is passed to a member accepting `obj` argument, e.g. `Method(o:obj)` will warn if called via `Method()`." )
    static member featureUseBangBindingValueDiscard() = (sprintf "Allows use! _ = ... in computation expressions" )
    static member featureBetterAnonymousRecordParsing() = (sprintf "Support for better anonymous record parsing" )
    static member featureScopedNowarn() = (sprintf "Support for scoped enabling / disabling of warnings by #warn and #nowarn directives, also inside modules" )
    static member featureErrorOnInvalidDeclsInTypeDefinitions() = (sprintf "Error when invalid declarations are used in type definitions." )
    static member featureAllowLetOrUseBangTypeAnnotationWithoutParens() = (sprintf "Allow let! and use! type annotations without requiring parentheses" )
    static member lexWarnDirectiveMustBeFirst() = (3874, sprintf "#nowarn/#warnon directives must appear as the first non-whitespace characters on a line" )
    static member lexWarnDirectiveMustHaveArgs() = (3875, sprintf "Warn directives must have warning number(s) as argument(s)" )
    static member lexWarnDirectivesMustMatch(a0 : System.String, a1 : System.Int32) = (3876, sprintf "There is another %s for this warning already in line %d." a0 a1)
    static member lexLineDirectiveMappingIsNotUnique(a0 : System.String, a1 : System.String) = (3877, sprintf "The file '%s' was also pointed to in a line directive in '%s'. Proper warn directive application may not be possible." a0 a1)
    static member tcAttributeIsNotValidForUnionCaseWithFields() = (3878, sprintf "This attribute is not valid for use on union cases with fields." )
    static member featureReturnFromFinal() = (sprintf "Support for ReturnFromFinal/YieldFromFinal in computation expressions to enable tailcall optimization when available on the builder." )
