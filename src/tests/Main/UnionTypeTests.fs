...
let ``Pattern matching json parse union cases still works``() =
    // Test IntType
    match jsonParse """{"tag":0,"a":1}""" with
    | IntType x -> x
    | _ -> failwith "unexpected"
    |> equal 1
    // Test StringType
    match jsonParse """{"tag":1,"a":"value1"}""" with
    | StringType x -> x
    | _ -> failwith "unexpected"
    |> equal "value1"
    // Test TupleType
    match jsonParse """{"tag":2,"a":"value1","b":2}""" with
    | TupleType(x, y) -> x, y
    | _ -> failwith "unexpected"
    |> fun (x, y) ->
        x |> equal "value1"
        y |> equal 2
    // Test ObjectType
    match jsonParse """{"tag":3,"a":{"Prop1":"value1","Prop2":2}}""" with
    | ObjectType(x) -> x
    | _ -> failwith "unexpected"
    |> fun x ->
