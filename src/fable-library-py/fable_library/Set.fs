...
module SetTree =
        ...
        if t2h > t1h + tolerance then // right is heavier than left
            ...
            | :? SetTreeNode<'T> as t2' ->
                ...
                if height t2'.Left > t1h + 1 then // balance left: combination
                    ...
                    | :? SetTreeNode<'T> as t2l -> mk (mk t1 v t2l.Left) t2l.Key (mk t2l.Right t2'.Key t2'.Right)
            ...
            | :? SetTreeNode<'T> as t1' ->
                ...
                if height t1'.Right > t2h + 1 then // balance right: combination
                    ...
                    | :? SetTreeNode<'T> as t1r -> mk (mk t1'.Left t1'.Key t1r.Left) t1r.Key (mk t1r.Right v t2)
                    | _ -> failwith "internal error: Set.rebalance"
                else
    ...
    let current i =
        if i.started then
            match i.stack with
            | None :: _ -> failwith "Please report error: Set iterator, unexpected stack for current"
            | Some t :: _ -> t.Key
            | [] -> alreadyFinished ()
        else
            notStarted ()
    ...
    let rec moveNext i =
        if i.started then
            match i.stack with
            | [] -> false
            | None :: rest -> failwith "Please report error: Set iterator, unexpected stack for moveNext"
            | Some t :: rest ->
                match t with
            ...
            | (Some x1 :: t1), _ ->
                ...
                | :? SetTreeNode<'T> as x1n ->
                    compareStacks
                        comparer
                        (x1n.Left
                         :: (SetTreeNode(x1n.Key, empty, x1n.Right, 0) :> SetTreeLeaf<'T> |> Some)
                         :: t1)
                        l2
            ...
            | _, (Some x2 :: t2) ->
                ...
                | :? SetTreeNode<'T> as x2n ->
                    compareStacks
                        ...
                        l1
                        (x2n.Left
                         :: (SetTreeNode(x2n.Key, empty, x2n.Right, 0) :> SetTreeLeaf<'T> |> Some)
                         :: t2)
...
type Set<[<EqualityConditionalOn>] 'T when 'T: comparison>(comparer: IComparer<'T>, tree: SetTree<'T>) =
    ...
    // Python Set protocol - explicit interface implementation (methods become attached)
    interface Fable.Core.Py.Set.ISet<'T> with
        member this.Contains(value) = this.Contains(value)
...
let singleton (value: 'T) ([<Inject>] comparer: IComparer<'T>) : Set<'T> = Set<'T>.Empty(comparer).Add value
...
let unionWith (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
    s2 |> Seq.iter (fun x -> s1.Add(x) |> ignore)
    s1

let newMutableSetWith (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
    Fable.Collections.MutableSet(s2, s1.Comparer)

let intersectWith (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
...
let exceptWith (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
    s2 |> Seq.iter (fun x -> s1.Remove(x) |> ignore)

let isSubsetOf (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
    let s2 = newMutableSetWith s1 s2
    s1 |> Seq.forall s2.Contains

let isSupersetOf (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) = s2 |> Seq.forall s1.Contains

let isProperSubsetOf (s1: Fable.Collections.MutableSet<'T>) (s2: 'T seq) =
    let s2 = newMutableSetWith s1 s2
