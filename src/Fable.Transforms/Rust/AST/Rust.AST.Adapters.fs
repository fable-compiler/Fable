...
type Option<'T> with
    ...
    member self.unwrap() =
        self.Value
    member self.unwrap_or(value: 'T) =
        Option.defaultValue value self
    member self.unwrap_or_else(f: unit -> 'T) =
        Option.defaultWith f self
    member self.and_then(f: 'T -> 'U option) =
        Option.bind f self
    member self.map(f: 'T -> 'U) =
        Option.map f self
    member self.iterate(f: 'T -> unit) =
        Option.iter f self

type System.Boolean with
    member self.then_some<'T>(t: 'T) =
        if self then Some t else None
...
type System.Collections.Generic.IList<'T> with
    ...
    member self.is_empty() =
        self.Count = 0
    member self.first() =
        if self.Count = 0 then None
        else Some(self.Item(0))
    member self.last() =
        if self.Count = 0 then None
        else Some(self.Item(self.Count - 1))
...
type System.Collections.Generic.IEnumerator<'T> with
    member self.next() =
        if self.MoveNext()
        then Some (self.Current)
        else None
    member self.enumerate() =
        { new System.Collections.Generic.IEnumerable<'T> with
            member x.GetEnumerator() = self
        interface System.Collections.IEnumerable with
            member x.GetEnumerator() = (self :> System.Collections.IEnumerator) }

type System.Collections.Generic.List<'T> with
    static member with_capacity(capacity: usize) =
        ResizeArray<'T>(capacity)
    ...
    member self.clone() =
        self.GetRange(0, self.Count)
    member self.map(f: 'T -> 'U) =
        self.ConvertAll(System.Converter(f))
    member self.push(item: 'T) =
        self.Add(item)
    member self.pop() =
    ...
    member self.extend(items: 'T seq) =
        self.AddRange(items)
    member self.split_first() =
        if self.Count = 0 then None
        else Some(self.Item(0), self.GetRange(1, self.Count - 1))
    member self.split_last() =
        if self.Count = 0 then None
        else Some(self.Item(self.Count - 1), self.GetRange(0, self.Count - 1))

    // VecDeque: // TODO: better impl
    member self.push_front(item: 'T) =
        self.Insert(0, item)
    member self.push_back(item: 'T) =
        self.Add(item)
    member self.front() =
        if self.Count > 0
        then Some(self.Item(0))
        else None
    member self.back() =
        self.last()
...
type System.String with
    ...
    member self.chars() =
        self.GetEnumerator()
    member self.repeat(n: usize) =
        String.replicate (int n) self
    member self.last() =
...
type System.Text.StringBuilder with
    ...
    member self.as_str() =
        self.ToString()
    member self.push_str(s: string) =
        self.Append(s) |> ignore
    member self.push(c: char) =
        self.Append(c) |> ignore
...
type Macros =
    ...
    static member assert_ne(actual, expected) =
        assert(actual <> expected)
