...
type Comments with
    static member new_(sm: SourceMap, filename: FileName, input: string): Comments =
        ...
        { sm = sm; comments = comments; current = 0 }

    member self.next(): Option<Comment> =
        // self.comments_.get(self.current).cloned()
        self.comments.pop()

    member self.trailing_comment   (span: Span,
                                    next_pos: Option<BytePos>): Option<Comment> =
...
let print_crate(sm: SourceMap,
    ...
    let mutable s: State =
        { s = pp.mk_printer(); comments_ = Some(Comments.new_(sm, filename, input)); ann = ann }
...
let doc_comment_to_string  (comment_kind: token.CommentKind,
...
type State with
    member self.strsep<'T> (sep: string,
                            space_before: bool,
                            b: pp.Breaks,
                            elts: Vec<'T>,
    ...
    member self.print_either_attributes(attrs: Vec<ast.Attribute>,
                                        kind: ast.AttrStyle,
                                        is_inline: bool,
        ...
        match item.args with
            | ast.MacArgs.Delimited(_, delim, tokens) ->
                self.print_mac_common(
                    ...
                    true,
                    span)
            | ast.MacArgs.Empty | ast.MacArgs.Eq(_) ->
    ...
    member self.print_mac_common   (header: Option<MacHeader>,
                                    has_bang: bool,
                                    ident: Option<Ident>,
    ...
    member self.token_kind_to_string_ext   (token_: token.TokenKind,
                                            convert_dollar_crate: Option<Span>): string =
    ...
    member self.comments(): Option<Comments> =
        self.comments_
    ...
    member self.print_generic_args(args: ast.GenericArgs, colons_before_params: bool) =
        ...
        match args with
            | ast.GenericArgs.AngleBracketed(data) ->
                self.s.word("<")
                self.commasep(pp.Breaks.Inconsistent, data.args, fun (s, arg) ->
                    match arg with
...
// type State with
    static member new_(): State =
        { s = pp.mk_printer(); comments_ = None; ann = NoAnn() }
    ...
    member self.print_type(ty: ast.Ty) =
        ...
        match ty.kind with
            ...
            | ast.TyKind.TraitObject(bounds, syntax) ->
                let prefix = if syntax = ast.TraitObjectSyntax.Dyn then "dyn" else ""
    ...
    member self.print_item_const   (ident: Ident,
    ...
    member self.print_associated_type  (ident: Ident,
    ...
    member self.print_item(item: ast.Item) =
        ...
        match item.kind with
            ...
            | ast.ItemKind.Mod(unsafety, mod_kind) ->
                self.head(self.to_string(fun (s) ->
    ...
    member self.print_enum_def(enum_definition: ast.EnumDef,
        generics: ast.Generics,
        ident: Ident,
        span: Span,
    ...
    member self.print_defaultness(defaultness: ast.Defaultness) =
        match defaultness with
            | ast.Defaultness.Default(_) ->
                self.s.word_nbsp("default")
            | _ -> ()

    member self.print_struct   (struct_def: ast.VariantData,
                                generics: ast.Generics,
                                ident: Ident,
                                span: Span,
        ...
        match struct_def with
            | ast.VariantData.Tuple(_) | ast.VariantData.Unit(_) ->
                match struct_def with
                | ast.VariantData.Tuple(_) ->
                    self.s.popen()
                    self.commasep(pp.Breaks.Inconsistent, struct_def.fields(), fun (s, field) ->
    ...
    member self.print_block_maybe_unclosed (blk: ast.Block,
    ...
    member self.cond_needs_par(expr: ast.Expr): bool =
        match expr.kind with
            ...
            | ast.ExprKind.Closure(_) | ast.ExprKind.Ret(_) | ast.ExprKind.Break(_) -> true
    ...
    member self.print_expr_repeat  (element: ast.Expr,
                                    count: ast.AnonConst,
                                    attrs: Vec<ast.Attribute>) =
    ...
    member self.print_expr_struct  (path: ast.Path,
    ...
    member self.print_expr_binary(op: ast.BinOp, lhs: ast.Expr, rhs: ast.Expr) =
        ...
        let left_prec =
            ...
            | (ast.ExprKind.Let _, _) when not(parser.needs_par_as_let_scrutinee(prec)) ->
                parser.PREC_FORCE_PAREN
            | _ -> left_prec
    ...
    member self.print_expr_addr_of (kind: ast.BorrowKind,
    ...
    member self.print_expr_outer_attr_style(expr: ast.Expr, is_inline: bool) =
        ...
        match expr.kind with
            ...
            | ast.ExprKind.Type(expr, ty) ->
                ...
                self.s.word_space(":")
                self.print_type(ty)
            | ast.ExprKind.Let(pat, scrutinee) ->
            ...
            | ast.ExprKind.InlineAsm(a) ->
                ...
                self.s.word("asm!")
                self.s.popen()
                self.commasep(pp.Breaks.Consistent, args, fun (s, arg) ->
                    match arg with
                    | AsmArg.Template(template) -> s.print_string(template, ast.StrStyle.Cooked)
                    | AsmArg.Operand(op) ->
                        let print_reg_or_class = fun (s: State, r: ast.InlineAsmRegOrRegClass) ->
                            match r with
                        ...
                        match op with
                            ...
                            | ast.InlineAsmOperand.Out (reg, late, expr) ->
                                s.s.word(if late then "lateout" else "out")
                                s.s.popen()
                                print_reg_or_class(s, reg)
                                s.s.pclose()
                                s.s.space()
                            ...
                            | ast.InlineAsmOperand.InOut (reg, late, expr) ->
                                s.s.word(if late then "inlateout" else "inout")
                                s.s.popen()
                                print_reg_or_class(s, reg)
                                s.s.pclose()
                                s.s.space()
                    ...
                    | AsmArg.Options(opts) ->
                        ...
                        s.commasep(pp.Breaks.Inconsistent, options, fun (s, opt) ->
                            s.s.word(opt)
                        )
                        s.s.pclose()
                )
            ...
            | ast.ExprKind.LlvmInlineAsm(a) ->
                ...
                self.commasep(pp.Breaks.Inconsistent, a.outputs, fun (s, out) ->
                ...
                self.commasep(pp.Breaks.Inconsistent, a.inputs, fun (s, (co, o)) ->
                ...
                self.commasep(pp.Breaks.Inconsistent, a.clobbers, fun (s, co) ->
                    s.print_symbol(co, ast.StrStyle.Cooked)
                )
                ...
                if not(options.is_empty()) then
                    self.s.space()
                    self.s.word_space(":")
                    self.commasep(pp.Breaks.Inconsistent, options, fun (s, co) ->
    ...
    member self.print_pat(pat: ast.Pat) =
        ...
        match pat.kind with
            ...
            | ast.PatKind.TupleStruct(path, elts) ->
                ...
                self.s.pclose()
            | ast.PatKind.Or(pats) ->
                self.strsep("|", true, pp.Breaks.Inconsistent, pats, fun (s, p) -> s.print_pat(p))
            | ast.PatKind.Path(None, path) ->
                self.print_path(path, true, 0)
    ...
    member self.print_fn_full  (sig_: ast.FnSig,
                                name: Ident,
                                generics: ast.Generics,
                                vis: ast.Visibility,
                                defaultness: ast.Defaultness,
    ...
    member self.print_fn   (decl: ast.FnDecl,
                            header: ast.FnHeader,
                            name: Option<Ident>,
                            generics: ast.Generics) =
    ...
    member self.print_fn_params_and_ret(decl: ast.FnDecl, is_closure: bool) =
        let (open_, close) = if is_closure then ("|", "|") else ("(", ")")
        self.s.word(open_)
    ...
    member self.print_lifetime_bounds  (lifetime: ast.Lifetime,
                                        bounds: ast.GenericBounds) =
    ...
    member self.print_generic_params(generic_params: Vec<ast.GenericParam>) =
        ...
        self.commasep(pp.Breaks.Inconsistent, generic_params, fun (s, param) ->
    ...
    member self.print_ty_fn(ext: ast.Extern,
        ...
        let generics: ast.Generics = {
            ...
            where_clause = {
                ...
                span = DUMMY_SP }
            span = DUMMY_SP }
        let header: ast.FnHeader = { ast.FnHeader.default_() with unsafety=unsafety; ext=ext }
        self.print_fn(decl, header, name, generics)
        self.s.end_()

    member self.maybe_print_trailing_comment   (span: Span,
