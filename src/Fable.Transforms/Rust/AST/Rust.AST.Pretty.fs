...
type Token with
    ...
    member self.is_hardbreak_tok(): bool =
        match self with
        | Token.Break({ offset = 0; blank_space = _SIZE_INFINITY }) -> true
        | _ -> false

// interface fmt.Display with // for Token
    member self.fmt(f: fmt.Formatter): fmt.Result =
        match self with
            | Token.String(s) -> f.write_str(format("STR({},{})", s, s.len()))
...
type Printer with
    ...
    member self.scan_begin(b: BeginToken) =
        ...
        self.scan_push({ token = Token.Begin(b); size = -self.right_total })
    ...
    member self.scan_end() =
        if self.scan_stack.is_empty() then
            ...
            self.advance_right()
            self.scan_push({ token = Token.End; size = -1 })
    ...
    member self.scan_break(b: BreakToken) =
        ...
        self.check_stack(0)
        self.scan_push({ token = Token.Break(b); size = -self.right_total })
    ...
    member self.scan_top(): usize =
        self.scan_stack.front().unwrap()

    member self.scan_pop_bottom(): usize =
    ...
    member self.get_top(): PrintStackElem =
        self.print_stack.last().unwrap_or(
            { offset = 0; pbreak = PrintStackBreak.Broken(Breaks.Inconsistent) }
        )
    ...
    member self.print_begin(b: BeginToken, l: isize) =
        if l > self.space_left then
            ...
            self.print_stack
                .push({ offset = col; pbreak = PrintStackBreak.Broken(b.breaks) })
        else
            debug("print Begin -> push fitting block")
            self.print_stack.push({ offset = 0; pbreak = PrintStackBreak.Fits })
    ...
    member self.print(token: Token, l: isize) =
        debug("print {} {} (remaining line space={})", token, l, self.space_left)
    ...
    member self.rbox(indent: usize, b: Breaks) =
        self.scan_begin({ offset = indent; breaks = b })

    /// Inconsistent breaking box
    member self.ibox(indent: usize) =
        self.rbox(indent, Breaks.Inconsistent)

    /// Consistent breaking box
    member self.cbox(indent: usize) =
        self.rbox(indent, Breaks.Consistent)

    member self.break_offset(n: usize, off: isize) =
        self.scan_break({ offset = off; blank_space = n })
    ...
    member self.spaces(n: usize) =
        self.break_offset(n, 0)

    member self.zerobreak() =
        self.spaces(0)

    member self.space() =
        self.spaces(1)

    member self.hardbreak() =
        self.spaces(_SIZE_INFINITY)
    ...
    static member hardbreak_tok_offset(off: isize): Token =
        Token.Break({ offset = off; blank_space = _SIZE_INFINITY })
