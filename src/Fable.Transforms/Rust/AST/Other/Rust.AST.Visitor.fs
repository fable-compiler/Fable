...
type VisitorImpl() =
  ...
  interface Visitor with
    ...
    member self.visit_name(_span: Span, _name: Symbol) =
        () // Nothing to do.
...
let walk_foreign_item(visitor: Visitor, item: ForeignItem) =
    match item with
    | { attrs=attrs; id=id; span=span; vis=vis; ident=ident; kind=kind } ->
...
let walk_global_asm(_: Visitor, _: GlobalAsm) =
    () // Empty!
...
let walk_generic_param(visitor: Visitor, param: GenericParam) =
    ...
    match param.kind with
        ...
        | GenericParamKind.Const(ty, _, default_) ->
            ...
            match default_ with
            | Some(default_) ->
                visitor.visit_anon_const(default_)
            | _ -> ()
...
let walk_stmt(visitor: Visitor, statement: Stmt) =
    match statement.kind with
        ...
        | StmtKind.Empty -> ()
        | StmtKind.MacCall({ mac=mac; style=_; attrs=attrs; tokens=_ }) ->
            visitor.visit_mac_call(mac)
