...
let makeRefFromMutableValue com ctx r t (value: Expr) =
    let getter = Delegate([], value, DelegateDetails.Create())

    let setter =
        let v = makeUniqueIdent ctx t "v"
        Delegate([ v ], Set(value, ValueSet, t, IdentExpr v, None), DelegateDetails.Create())

    Helper.LibCall(com, "types", "FSharpRef", t, [ getter; setter ], isConstructor = true)

let makeRefFromMutableField com ctx r t callee key =
    let getter = Delegate([], Get(callee, FieldGet(key, true), t, r), DelegateDetails.Create())

    let setter =
        let v = makeUniqueIdent ctx t "v"
        Delegate([ v ], Set(callee, FieldSet(key), t, IdentExpr v, r), DelegateDetails.Create())
...
let makeRefFromMutableFunc com ctx r t (value: Expr) =
    let getter =
        ...
        Delegate([], value, DelegateDetails.Create())
    ...
    let setter =
        ...
        Delegate([ v ], value, DelegateDetails.Create())
...
let toFloat com (ctx: Context) r targetType (args: Expr list) : Expr =
    ...
    | Char ->
        //Helper.InstanceCall(args.Head, "charCodeAt", Number(Int32, NumberDetails.None), [ makeIntConst 0 ])
...
let toDecimal com (ctx: Context) r targetType (args: Expr list) : Expr =
    ...
    | Number(kind,_) ->
        ...
        | BigInt -> Helper.LibCall(com, "big_int", castBigIntMethod targetType, targetType, args)
        | Int64 | UInt64 ->
            Helper.LibCall(com, "long", "toNumber", Number(Float64, NumberDetails.None), args)
            |> makeDecimalFromExpr com r targetType
...
let fastIntFloor expr =
    ...
    makeUnOp None (Number(Int32, NumberDetails.None)) inner UnaryNotBitwise
...
let toLong com (ctx: Context) r (unsigned: bool) targetType (args: Expr list) : Expr =
    ...
    | Number(kind,_) ->
        match kind with
        | Decimal ->
            let n = Helper.LibCall(com, "decimal", "toNumber", Number(Float64, NumberDetails.None), args)
...
let toInt com (ctx: Context) r targetType (args: Expr list) =
    ...
    let emitCast typeTo arg =
        match typeTo with
        | Int8 -> emitExpr None (Number(Int8, NumberDetails.None)) [ arg ] "(int($0) + 0x80 & 0xFF) - 0x80"
        | Int16 -> emitExpr None (Number(Int16, NumberDetails.None)) [ arg ] "(int($0) + 0x8000 & 0xFFFF) - 0x8000"
        | Int32 -> fastIntFloor arg
        | UInt8 -> emitExpr None (Number(UInt8, NumberDetails.None)) [ arg ] "int($0+0x100 if $0 < 0 else $0) & 0xFF"
        | UInt16 -> emitExpr None (Number(UInt16, NumberDetails.None)) [ arg ] "int($0+0x10000 if $0 < 0 else $0) & 0xFFFF"
        | UInt32 -> emitExpr None (Number(UInt32, NumberDetails.None)) [ arg ] "int($0+0x100000000 if $0 < 0 else $0)"
...
let round com (args: Expr list) =
    ...
    | Number(Decimal,_) ->
        let n =
            Helper.LibCall(com, "decimal", "toNumber", Number(Float64, NumberDetails.None), [ args.Head ])

        let rounded = Helper.LibCall(com, "util", "round", Number(Float64, NumberDetails.None), [ n ])

        rounded :: args.Tail
    | Number ((Float32|Float64), _) ->
        let rounded =
            Helper.LibCall(com, "util", "round", Number(Float64, NumberDetails.None), [ args.Head ])
...
let applyOp (com: ICompiler) (ctx: Context) r t opName (args: Expr list) argTypes genArgs =
    ...
    let nativeOp opName argTypes args =
        ...
        | Operators.bitwiseAnd, [ left; right ] ->
            binOp BinaryAndBitwise left right
            |> truncateUnsigned
        | Operators.bitwiseOr, [ left; right ] ->
            binOp BinaryOrBitwise left right
            |> truncateUnsigned
        | Operators.exclusiveOr, [ left; right ] ->
            binOp BinaryXorBitwise left right
            |> truncateUnsigned
...
let identityHash com r (arg: Expr) =
    ...
    Helper.LibCall(com, "Util", methodName, Number(Int32, NumberDetails.None), [ arg ], ?loc = r)
...
and compare (com: ICompiler) ctx r (left: Expr) (right: Expr) =
    match left.Type with
    | Number (Decimal,_) ->
        Helper.LibCall(com, "decimal", "compare", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
    | Number (BigInt,_) ->
        Helper.LibCall(com, "big_int", "compare", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
    | Builtin (BclGuid
    | BclTimeSpan)
    | Boolean
    | Char
    | String
    | Number _ -> Helper.LibCall(com, "util", "comparePrimitives", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
    | Builtin (BclDateTime
    | BclDateTimeOffset) -> Helper.LibCall(com, "date", "compare", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
    | DeclaredType _ -> Helper.LibCall(com, "util", "compare", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
    | Array t ->
        let f = makeComparerFunction com ctx t
        Helper.LibCall(com, "array", "compareWith", Number(Int32, NumberDetails.None), [ f; left; right ], ?loc = r)
    | List _ -> Helper.LibCall(com, "util", "compare", Number(Int32, NumberDetails.None), [ left; right ], ?loc = r)
...
and makeComparerFunction (com: ICompiler) ctx typArg =
    ...
    Delegate([ x; y ], body, DelegateDetails.Create())
...
let makeEqualityComparer (com: ICompiler) ctx typArg =
    ...
    objExpr [ "Equals", Delegate([ x; y ], equals com ctx None true (IdentExpr x) (IdentExpr y), DelegateDetails.Create())
              "GetHashCode", Delegate([ x ], structuralHash com None (IdentExpr x), DelegateDetails.Create()) ]
...
let makeGenericAverager (com: ICompiler) ctx t =
    let divideFn =
        let x = makeUniqueIdent ctx t "x"
        let i = makeUniqueIdent ctx (Number(Int32, NumberDetails.None)) "i"

        let body =
            applyOp com ctx None t Operators.divideByInt [ IdentExpr x; IdentExpr i ] [ t; Number(Int32, NumberDetails.None) ] []

        Delegate([ x; i ], body, DelegateDetails.Create())
...
let tryEntityRef (com: Compiler) entFullName =
    ...
    | Types.timeoutException ->
        makeImportLib com Any "TimeoutException" "SystemException"
        |> Some
    | _ -> None
...
let fableCoreLib (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "Fable.Core.JsInterop", _ ->
        ...
        | "importValueDynamic", [ arg ] ->
            let dynamicImport selector path =
                ...
                | selector ->
                    let selector =
                        let m = makeIdent "m"
                        Delegate([ m ], Get(IdentExpr m, ExprGet selector, Any, None), DelegateDetails.Create())
        ...
        | "op_DynamicAssignment", [ callee; prop; MaybeLambdaUncurriedAtCompileTime value ] -> setExpr r callee prop value |> Some
        | ("op_Dollar"
          | "createNew" as m),
          callee :: args ->
...
let newReference com r t value =
    Helper.LibCall(com, "types", "FSharpRef", t, [ value ], isConstructor = true, ?loc = r)
...
let fsFormat (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "PrintFormatToStringThen", _, _ ->
        ...
        | [ _ ] ->
            ...
            |> Some
        | [ cont; fmt ] ->
            Helper.InstanceCall(fmt, "cont", t, [ cont ])
            |> Some
        | _ -> None
    ...
    | "PrintFormatThen", _, arg :: callee :: _ ->
        Helper.InstanceCall(callee, "cont", t, [ arg ])
        |> Some
...
let operators (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ("ToSingle"
      | "ToDouble"),
      _ -> toFloat com ctx r t args |> Some
    ...
    | ("PrintFormatToString" // sprintf
      ...
      | "PrintFormat"
      | "PrintFormatLine" // printf / printfn
    ...
    | (Operators.inequality
      | "Neq"),
      [ left; right ] -> equals com ctx r false left right |> Some
    | (Operators.equality
      | "Eq"),
...
let strings (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "Equals", None, [ x; y; kind ] ->
        let left =
            Helper.LibCall(com, "string", "compare", Number(Int32, NumberDetails.None), [ x; y; kind ])
    ...
    | "Contains", Some c, arg :: _ ->
        ...
        let left = Helper.InstanceCall(c, "find", Number(Int32, NumberDetails.None), [ arg ])

        makeEqOp r left (makeIntConst 0) BinaryGreaterOrEqual
        |> Some
    | "StartsWith", Some c, [ _str ] ->
        let left = Helper.InstanceCall(c, "find", Number(Int32, NumberDetails.None), args)
    ...
    | "IndexOf", Some c, _ ->
        ...
        | [ ExprType Char ]
        | [ ExprType String ]
        | [ ExprType Char; ExprType (Number (Int32, NumberDetails.None)) ]
        | [ ExprType String; ExprType (Number (Int32, NumberDetails.None)) ] ->
            Helper.InstanceCall(c, "find", t, args, i.SignatureArgTypes, ?loc = r)
            |> Some
        | _ ->
    ...
    | "LastIndexOf", Some c, _ ->
        ...
        | [ ExprType Char ]
        | [ ExprType String ] ->
            Helper.InstanceCall(c, "rfind", t, args, i.SignatureArgTypes, ?loc = r)
            |> Some
        | [ ExprType Char as str; ExprType (Number (Int32, NumberDetails.None)) as start ]
        | [ ExprType String as str; ExprType (Number (Int32, NumberDetails.None)) as start ] ->
            Helper.InstanceCall(
                c,
                "rfind",
                t,
                [ str
                  Value(NumberConstant(0.0, Int32, NumberDetails.None), None)
                  start ],
                i.SignatureArgTypes,
                ?loc = r
            )
    ...
    | ("Trim"
      ...
      _ ->
        ...
        match args with
        | [] ->
            Helper.InstanceCall(c, methName, t, [], i.SignatureArgTypes, ?loc = r)
            |> Some
        | head :: tail ->
            let spread =
    ...
    | "Split", Some c, _ ->
        ...
        // Optimization
        | [] ->
            Helper.InstanceCall(c, "split", t, [ makeStrConst "" ])
            |> Some
        ...
        | [ Value (NewArray ([ separator ], _), _) ] ->
            ...
            |> Some
        | [arg1; ExprType(Number(_, NumberDetails.IsEnum _)) as arg2] ->
            let arg1 =
    ...
    | "Join", None, _ ->
        ...
        Helper.LibCall(com, "string", methName, t, args, ?loc = r)
        |> Some
    | "Concat", None, _ ->
        match i.SignatureArgTypes with
        | [ Array _ | IEnumerable ] ->
...
let stringModule (com: ICompiler) (ctx: Context) r t (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | ("Iterate"
      ...
      _ ->
        ...
        let args =
            args
            |> List.replaceLast (fun e -> stringToCharArray e.Type e)
...
let resizeArrays (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor", _, [ ExprType (Number _) ] -> makeArray Any [] |> Some
    // Optimize expressions like `ResizeArray [|1|]` or `ResizeArray [1]`
    | ".ctor", _, [ ArrayOrListLiteral (vals, _) ] -> makeArray Any vals |> Some
    | ".ctor", _, args ->
    ...
    | "FindLastIndex", Some ar, [ arg ] ->
        ...
        |> Some
    | "ForEach", Some ar, [ arg ] ->
        Helper.LibCall(com, "array", "iterate", t, [ arg; ar ], ?loc = r)
        |> Some
    ...
    | "Find", Some ar, [ arg ] ->
        ...
        |> Some
    | "Exists", Some ar, [ arg ] ->
        let left = Helper.InstanceCall(ar, "index", Number(Int32, NumberDetails.None), [ arg ], ?loc = r)
    ...
    | "RemoveAt", Some ar, [ idx ] ->
        ...
        |> Some
    | "Reverse", Some ar, [] ->
        Helper.InstanceCall(ar, "reverse", t, args, ?loc = r)
        |> Some
    | "Sort", Some ar, [] ->
        let compareFn =
            (genArg com ctx r 0 i.GenericArgs)
            |> makeComparerFunction com ctx

        Helper.InstanceCall(ar, "sort", t, [ compareFn ], ?loc = r)
        |> Some
    | "Sort", Some ar, [ ExprType (DelegateType _) ] ->
        Helper.InstanceCall(ar, "sort", t, args, ?loc = r)
        |> Some
    | "Sort", Some ar, [ arg ] ->
...
let arrays (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "get_Length", Some arg, _ ->
        Helper.GlobalCall("len", t, [ arg ], [ t ], ?loc = r)
        |> Some
...
let arrayModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "IsEmpty", [ ar ] ->
        eq (getAttachedMemberWith r (Number(Int32, NumberDetails.None)) ar "length") (makeIntConst 0)
        |> Some
    ...
    | "SortInPlaceWith", args ->
        ...
        Helper.InstanceCall(thisArg, meth, t, args, argTypes, ?loc = r)
        |> Some
...
let lists (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ReplaceName [ "get_Head", "head"; "get_Tail", "tail"; "get_Item", "item"; "get_Length", "length"; "GetSlice", "getSlice" ] methName,
      Some x,
      _ ->
        let args =
            match args with
    ...
    | "get_Empty", None, _ ->
        NewList(None, (genArg com ctx r 0 i.GenericArgs))
        |> makeValue r
        |> Some
    | "Cons", None, [ h; t ] ->
...
let results (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | ("Bind"
    | "Map"
    | "MapError") as meth -> Some("Result_" + meth)
    | _ -> None
...
let optionModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | ("OfObj"
      | "OfNullable"),
      _ ->
        Helper.LibCall(com, "option", "ofNullable", t, args, ?loc = r)
        |> Some
    | ("ToObj"
      | "ToNullable"),
      _ ->
        Helper.LibCall(com, "option", "toNullable", t, args, ?loc = r)
        |> Some
    ...
    | "ToList", [ arg ] ->
        ...
        Helper.LibCall(com, "list", "ofArray", t, args, ?loc = r)
        |> Some
...
let decimals (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor", [ arg ] ->
        match arg.Type with
        | Array (Number (Int32, NumberDetails.None)) ->
...
let bigints (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | None, "DivRem" ->
        ...
        |> Some
    | None, meth when meth.StartsWith("get_") -> Helper.LibValue(com, "big_int", meth, t) |> Some
    | callee, meth ->
        let args =
...
let languagePrimitives (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ("GenericHash"
      ...
      [ arg ] -> structuralHash com r arg |> Some
    | ("FastHashTuple2"
      | "FastHashTuple3"
      | "FastHashTuple4"
      | "FastHashTuple5"
    ...
    | ("GenericComparison"
      ...
      [ left; right ] -> compare com ctx r left right |> Some
    | ("FastCompareTuple2"
      | "FastCompareTuple3"
      | "FastCompareTuple4"
      | "FastCompareTuple5"
    ...
    | ("PhysicalHash"
      ...
      [ arg ] ->
        Helper.LibCall(com, "util", "physicalHash", Number(Int32, NumberDetails.None), [ arg ], ?loc = r)
        |> Some
    ...
    | ("ParseInt32"
      | "ParseUInt32"),
      [ arg ] -> toInt com ctx r t [ arg ] |> Some
...
let intrinsicFunctions (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ("GetArraySlice"
      ...
      [ ar; lower; upper ] ->
        let upper =
            match upper with
            | Value (NewOption (None, _, _), _) -> getExpr None (Number(Int32, NumberDetails.None)) ar (makeStrConst "length")
    ...
    | ("TypeTestGeneric"
      ...
      None,
      [ expr ] ->
        Test(expr, TypeTest((genArg com ctx r 0 i.GenericArgs)), r)
        |> Some
...
let dictionaries (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor", _ ->
        ...
        | ([]
          | [ Number _ ]),
          _ ->
            makeDictionary com ctx r t (makeArray Any [])
            |> Some
...
let hashSets (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "GetEnumerator", Some c, _ -> getEnumerator com r t c |> Some
    | "Add", Some c, [ arg ] ->
        Helper.LibCall(com, "map_util", "addToSet", t, [ arg; c ], ?loc = r)
        |> Some
    | ("IsProperSubsetOf"
      | "IsProperSupersetOf"
      | "UnionWith"
      ...
      | "ExceptWith"
      | "IsSubsetOf"
      | "IsSupersetOf" as meth),
      Some c,
      args ->
        let meth = Naming.lowerFirst meth
        let args = injectArg com ctx r "Set" meth i.GenericArgs args

        Helper.LibCall(com, "Set", meth, t, c :: args, ?loc = r)
        |> Some
    // | "CopyTo" // TODO!!!
    // | "SetEquals"
    // | "Overlaps"
...
let enums (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | Some this, "HasFlag", [ arg ] ->
        ...
        makeBinOp r (Number(Int32, NumberDetails.None)) this arg BinaryAndBitwise
    ...
    | None,
      Patterns.DicContains (dict [ "Parse", "parseEnum"
                                   ...
                                   "GetUnderlyingType", "getEnumUnderlyingType" ])
                           meth,
      args ->
        let args =
...
let dates (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor" ->
        ...
        | _ ->
            ...
            | 7, Number(_, NumberDetails.IsEnum ent) when ent.FullName = "System.DateTimeKind" ->
                let args = (List.take 6 args) @ [ makeIntConst 0; last ]

                let argTypes =
                    (List.take 6 i.SignatureArgTypes)
                    @ [ Number(Int32, NumberDetails.None); last.Type ]

                Helper.LibCall(com, "Date", "create", t, args, argTypes, ?loc = r)
                |> Some
            | _ ->
    ...
    | "get_DateTime" ->
        ...
        Helper.LibCall(com, "Date", "fromDateTimeOffset", t, [ thisArg.Value; kind ], [ thisArg.Value.Type; kind.Type ], ?loc = r)
        |> Some
    | "FromUnixTimeSeconds"
    | "FromUnixTimeMilliseconds" ->
        let value =
            Helper.LibCall(com, "Long", "toNumber", Number(Float64, NumberDetails.None), args, i.SignatureArgTypes)
        ...
        Helper.LibCall(
            ...
            [ value.Type; Number(Int32, NumberDetails.None) ],
            ?loc = r
        )
        |> Some
    | "ToUnixTimeSeconds"
    | "ToUnixTimeMilliseconds" ->
        ...
        Helper.LibCall(com, "Long", "fromNumber", t, args, ?loc = r)
        |> Some
    | "get_Ticks" ->
        ...
        | Some c, [ ticks ] ->
            let ms =
                Helper.LibCall(
                    ...
                    [ ticks.Type; Number(Int32, NumberDetails.None) ]
                )

            let ms =
                Helper.LibCall(com, "long", "toNumber", Number(Float64, NumberDetails.None), [ ms ], [ ms.Type ])

            Helper.LibCall(com, moduleName, "addMilliseconds", Number(Float64, NumberDetails.None), [ c; ms ], [ c.Type; ms.Type ], ?loc = r)
            |> Some
        | _ -> None
    | meth ->
        let meth =
...
let timeSpans (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "ToString" when (args.Length = 2) ->
        ...
        | StringConst "g"
        | StringConst "G" ->
            Helper.LibCall(com, "time_span", "toString", t, args, i.SignatureArgTypes, ?thisArg = thisArg, ?loc = r)
            |> Some
        | _ ->
...
let cancels (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor" ->
        Helper.LibCall(com, "async_", "createCancellationToken", t, args, i.SignatureArgTypes)
        |> Some
    | "get_Token" -> thisArg
    | "Cancel"
    | "CancelAfter"
...
let regex com (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor" ->
        Helper.LibCall(com, "RegExp", "create", t, args, i.SignatureArgTypes, ?loc = r)
        |> Some
    | "get_Options" ->
...
let enumerables (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (_: Expr list) =
    ...
    | Some callee, "get_Count" ->
        Helper.LibCall(com, "Seq", "length", t, [ callee ], ?loc = r)
        |> Some
...
let mailbox (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | None ->
        match i.CompiledName with
        | ".ctor" ->
            Helper.LibCall(com, "mailbox_processor", "MailboxProcessor", t, args, i.SignatureArgTypes, isConstructor = true, ?loc = r)
            |> Some
        | "Start" ->
...
let asyncs com (ctx: Context) r t (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "Catch" ->
        Helper.LibCall(com, "async_", "catchAsync", t, args, i.SignatureArgTypes, ?loc = r)
        |> Some
...
let guids (com: ICompiler) (ctx: Context) (r: SourceLocation option) t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "ToString" when (args.Length = 0) ->
        Helper.GlobalCall("str", t, [ thisArg.Value ], ?loc = r)
        |> Some
    | "ToString" when (args.Length = 1) ->
        match args with
...
let types (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
        ...
        | Some (Value (TypeInfo(exprType, _), exprRange) as thisArg) ->
            ...
            | "GetInterface" ->
                ...
                |> Option.map (fun (e, genArgs, name, ignoreCase) ->
                    ...
                    let genMap =
                        List.zip (e.GenericParameters |> List.map (fun p -> p.Name)) genArgs
                        |> Map
            ...
            | "get_IsEnum" ->
                match exprType with
                | Number(_, NumberDetails.IsEnum _) -> true
                | _ -> false
                |> BoolConstant
                |> makeValue r
                |> Some
    ...
    | None, Some thisArg ->
        ...
        | "IsInstanceOfType" ->
            ...
            Helper.LibCall(com, "Reflection", meth, t, thisArg :: args, ?loc = r)
            |> Some
        | _ -> None
    | None, None -> None
...
let tryType =
    ...
    | Number(kind, details) ->
        let f =
            ...
            | BigInt -> bigints
            | _ -> parseNum
        Some(getNumberFullName false kind details, f, [])
