...
and private getZero (com: ICompiler) ctx (t: Type) =
    match t with
...
let bclType (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName, thisArg, args with
    | "ToString", Some callee, [] -> Helper.InstanceCall(callee, i.CompiledName, t, args, ?loc = r) |> Some
...
let arrayModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    let createArray size value =
        ...
        | _ ->
            $"Expecting an array type but got %O{t}"
            |> addErrorAndReturnNull com ctx.InlinePath r
...
/// given, warning once per discarded argument. Two codes rather than one bundled message so a
/// user who accepts invariant parsing can still be told about a discarded style, and vice versa.
let private ignoreFormatProvider com (ctx: Context) r meth args =
