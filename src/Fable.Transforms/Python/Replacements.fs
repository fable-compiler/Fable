...
type Helper =
    ...
    static member InstanceCall(callee: Expr, memb: string, returnType: Type, args: Expr list, ?argTypes: Type list, ?loc: SourceLocation) =
        let callee = getAttachedMember callee memb
        let info = defaultArg argTypes [] |> makeCallInfo None args
        Call(callee, info, returnType, loc)

    static member Application(callee: Expr, returnType: Type, args: Expr list, ?argTypes: Type list, ?loc: SourceLocation) =
        let info = defaultArg argTypes [] |> makeCallInfo None args
        Call(callee, info, returnType, loc)
    ...
    static member LibCall
        ...
        Call(
            ...
            { info with
                ...
                IsConstructor = defaultArg isPyConstructor false },
            returnType,
            loc
        )
...
module Helpers =
    ...
    let getIdentUniqueName (ctx: Context) name =
        ...
        ctx.UsedNamesInDeclarationScope.Add(name)
        |> ignore
    ...
    let resolveArgTypes argTypes (genArgs: (string * Type) list) =
        ...
        |> List.map (function
            | GenericParam (name, _) as t ->
                genArgs
                |> List.tryPick (fun (name2, t) -> if name = name2 then Some t else None)
                |> Option.defaultValue t
            | t -> t)

    let asOptimizable optimization =
        function
    ...
    let genArg (com: ICompiler) (ctx: Context) r i (genArgs: (string * Type) list) =
        ...
        |> Option.defaultWith (fun () ->
            "Couldn't find generic argument in position "
            + (string i)
...
let (|BuiltinDefinition|_|) =
    ...
    | (Naming.StartsWith Types.choiceNonGeneric genArgs) ->
        List.replicate (int genArgs.[1..]) Any
        |> FSharpChoice
        |> Some
    | _ -> None
...
let rec namesof com ctx acc e =
    ...
    | [], NestedLambda (args, Call (IdentExpr ident, info, _, _), None) ->
        if List.sameLength args info.Args
...
let (|ArrayOrListLiteral|_|) =
    function
    | MaybeCasted (Value ((NewArray (vals, t)
                          | ListLiteral (vals, t)),
                          _)) -> Some(vals, t)
    | _ -> None
...
let (|NewAnonymousRecord|_|) =
    ...
    | Value (NewAnonymousRecord (exprs, fieldNames, genArgs), r) -> Some([], exprs, fieldNames, genArgs, r)
    | _ -> None
...
let makeUniqueIdent ctx t name =
    Helpers.getIdentUniqueName ctx name
    |> makeTypedIdent t
...
let makeFloat32 r (x: float32) =
    Helper.GlobalCall(
        "math",
        Number(Float32, None),
        [ NumberConstant(float x, Float32, None)
          |> makeValue r ],
        memb = "fround"
    )
...
let makeTypeConst com r (typ: Type) (value: obj) =
    ...
    | Number (UInt16, uom), (:? uint16 as x) ->
        NumberConstant(float x, UInt16, uom)
        |> makeValue r
    | Number (Int32, uom), (:? int as x) -> NumberConstant(float x, Int32, uom) |> makeValue r
    | Number (UInt32, uom), (:? uint32 as x) ->
        NumberConstant(float x, UInt32, uom)
        |> makeValue r
    // Float types
    | Number (Float32, uom), (:? float32 as x) ->
        NumberConstant(float x, Float32, uom)
        |> makeValue r
    | Number (Float64, uom), (:? float as x) ->
        NumberConstant(float x, Float64, uom)
        |> makeValue r
    // Enums
    | Enum _, (:? int64)
    ...
    | Enum e, (:? byte as x) ->
        EnumConstant(
            NumberConstant(float x, UInt8, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    | Enum e, (:? sbyte as x) ->
        EnumConstant(
            NumberConstant(float x, Int8, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    | Enum e, (:? int16 as x) ->
        EnumConstant(
            NumberConstant(float x, Int16, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    | Enum e, (:? uint16 as x) ->
        EnumConstant(
            NumberConstant(float x, UInt16, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    | Enum e, (:? int as x) ->
        EnumConstant(
            NumberConstant(float x, Int32, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    | Enum e, (:? uint32 as x) ->
        EnumConstant(
            NumberConstant(float x, UInt32, None)
            |> makeValue None,
            e
        )
        |> makeValue r
    ...
    | Array (Number (kind, uom)), (:? (byte []) as arr) ->
        let values =
            arr
            |> Array.map (fun x ->
                NumberConstant(float x, kind, uom)
                |> makeValue None)
            |> Seq.toList
...
let toString com (ctx: Context) r (args: Expr list) =
    ...
    | head :: tail ->
        ...
        | Builtin (BclInt64
        | BclUInt64
        | BclBigInt) -> Helper.LibCall(com, "util", "int64_to_string", String, args)
        | Number (Int8, _)
...
let toInt com (ctx: Context) r targetType (args: Expr list) =
    let transformEnumType =
        function
        | Enum _ -> Number(Int32, None)
        | t -> t
...
let rec findInScope (scope: FSharp2Fable.Scope) identName =
    ...
    | (_, ident2, expr) :: prevScope ->
        if identName = ident2.Name then
            ...
            findInScope prevScope identName
...
let isCompatibleWithJsComparison =
    function
    | Builtin (BclInt64
    | BclUInt64
    | BclDecimal
...
and compare (com: ICompiler) ctx r (left: Expr) (right: Expr) =
    ...
    | List _ -> Helper.LibCall(com, "util", "compare", Number(Int32, None), [ left; right ], ?loc = r)
    | Tuple _ -> Helper.LibCall(com, "util", "compareArrays", Number(Int32, None), [ left; right ], ?loc = r)
    | _ -> Helper.LibCall(com, "util", "compare", Number(Int32, None), [ left; right ], ?loc = r)
...
let rec getZero (com: ICompiler) ctx (t: Type) =
    ...
    | Builtin (BclInt64
    | BclUInt64) as t -> Helper.LibCall(com, "Long", "fromInt", t, [ makeIntConst 0 ])
...
let getOne (com: ICompiler) ctx (t: Type) =
    match t with
    | Builtin (BclInt64
    | BclUInt64) as t -> Helper.LibCall(com, "long", "fromInt", t, [ makeIntConst 1 ])
...
let makeGenericAverager (com: ICompiler) ctx t =
    let divideFn =
        ...
        let body =
            applyOp com ctx None t Operators.divideByInt [ IdentExpr x; IdentExpr i ] [ t; Number(Int32, None) ] []
...
let arrayModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "IsEmpty", [ ar ] ->
        eq (getAttachedMemberWith r (Number(Int32, None)) ar "length") (makeIntConst 0)
        |> Some
...
let languagePrimitives (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "EnumOfValue", [ arg ] ->
        ...
        | _ ->
            "EnumOfValue only works if the enum type is known at compile time, try inlining the function"
            |> addErrorAndReturnNull com ctx.InlinePath r
            |> Some
    | "EnumToValue", [ arg ] ->
        match arg with
    ...
    | ("PhysicalHash"
      ...
      [ arg ] ->
        Helper.LibCall(com, "util", "physicalHash", Number(Int32, None), [ arg ], ?loc = r)
        |> Some
...
let intrinsicFunctions (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "CreateInstance", None, _ ->
        ...
        | DeclaredType (ent, _) ->
            ...
            Helper.PyConstructorCall(pyConstructor com ent, t, [], ?loc = r)
            |> Some
        | t ->
...
let exceptions (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName, thisArg with
    | ".ctor", _ ->
        Helper.PyConstructorCall(makeIdentExpr "Exception", t, args, ?loc = r)
        |> Some
...
let dates (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "FromUnixTimeMilliseconds" ->
        let value =
            Helper.LibCall(com, "Long", "toNumber", Number(Float64, None), args, i.SignatureArgTypes)
        ...
        | Some c, [ ticks ] ->
            let ms =
                Helper.LibCall(
                    ...
                    [ ticks.Type; Number(Int32, None) ]
                )

            let ms =
                Helper.LibCall(com, "long", "toNumber", Number(Float64, None), [ ms ], [ ms.Type ])

            Helper.LibCall(com, moduleName, "addMilliseconds", Number(Float64, None), [ c; ms ], [ c.Type; ms.Type ], ?loc = r)
            |> Some
        | _ -> None
    | meth ->
        let meth =
...
let timers (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName, thisArg, args with
    | ".ctor", _, _ ->
        Helper.LibCall(com, "timer", "Timer", t, args, i.SignatureArgTypes, isPyConstructor = true, ?loc = r)
        |> Some
...
let events (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName, thisArg with
    | ".ctor", _ ->
        Helper.LibCall(com, "Event", "default", t, args, i.SignatureArgTypes, isPyConstructor = true, ?loc = r)
        |> Some
...
let mailbox (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | None ->
        match i.CompiledName with
        | ".ctor" ->
            Helper.LibCall(com, "mailbox_processor", "MailboxProcessor", t, args, i.SignatureArgTypes, isPyConstructor = true, ?loc = r)
            |> Some
        | "Start" ->
...
let uris (com: ICompiler) (ctx: Context) (r: SourceLocation option) t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName with
    | ".ctor" ->
        Helper.LibCall(com, "Uri", "Uri", t, args, i.SignatureArgTypes, isPyConstructor = true, ?loc = r)
        |> Some
    | "UnescapeDataString" ->
...
let laziness (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | (".ctor"
      ...
      _,
      _ ->
        Helper.LibCall(com, "Util", "Lazy", t, args, i.SignatureArgTypes, isPyConstructor = true, ?loc = r)
        |> Some
    | "CreateFromValue", _, _ ->
