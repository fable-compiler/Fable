...
//    |  "NullArg"         // nullArg
    ...
    | "Truncate", _ ->
        ...
        | _ -> Helper.GlobalCall("Math", t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, memb="trunc", ?loc=r) |> Some
    | "Sign", _ ->
...
let getEnumerator (_com: ICompiler) r t expr =
    getFieldWith r t expr "iterator"
...
let strings (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor", _, fstArg::_ ->
        ...
        | Array _ ->
            ...
            | [chars; startIdx; count] -> Helper.GlobalCall("String", t, chars::(toStartEndIndices startIdx count), memb="fromCharCodes", ?loc=r) |> Some // String(char[], int, int)
            | _ -> "Unexpected arguments in System.String constructor."
                   |> addErrorAndReturnNull com ctx.InlinePath r |> Some
        | _ ->
    ...
    | "Split", Some c, _ ->
        ...
        | [] | [Value(NewArray(ArrayValues [],_,_),_)] ->
            Helper.LibCall(com, "String", "split", t, [], ?thisArg=thisArg, ?loc=r) |> Some
        ...
        | arg1::restArgs ->
            ...
            let args =
                ...
                | _ -> arg1::restArgs
            Helper.LibCall(com, "String", meth, t, args, ?thisArg=thisArg, ?loc=r) |> Some
    | "Join", None, args ->
        match args with
        | [separator; arg] ->
            let arg =
    ...
    | "CompareTo", Some x, [y] ->
        Helper.LibCall(com, "String", "compare", t, [x; y], ?loc=r) |> Some
    | "Compare", None, args ->
        let meth =
            match args with
            | [_x; _y]
    ...
    | Patterns.SetContains implementedStringFunctions, thisArg, args ->
        Helper.LibCall(com, "String", Naming.lowerFirst i.CompiledName, t, args,
            i.SignatureArgTypes, genArgs=i.GenericArgs, ?thisArg=thisArg, ?loc=r) |> Some
    | _ -> None
...
//        Helper.LibCall(com, "Option", "defaultArg", t, [opt; defaultof com ctx t], ?loc=r) |> Some
    ...
    | "GetRange", Some ar, [idx; cnt] ->
        Helper.LibCall(com, "Array", "getSubArray", t, [ar; idx; cnt], genArgs=i.GenericArgs, ?loc=r) |> Some
    ...
    | "RemoveRange", Some ar, [startIdx; count] ->
        Helper.InstanceCall(ar, "removeRange", t, toStartEndIndices startIdx count, ?loc=r) |> Some
...
let arrayModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "IsEmpty", [ar] ->
        getFieldWith r t ar "isEmpty" |> Some
    | "CopyTo", args ->
...
let options (com: ICompiler) (_: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match i.CompiledName, thisArg with
    | "get_Value", Some c -> forceOptionValue r t c |> Some
...
let optionModule (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "GetValue", [c] -> forceOptionValue r t c |> Some
...
let parseNum (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "IsPositiveInfinity", [_] when isFloat ->
        Helper.LibCall(com, "Double", "isPositiveInfinity", t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
...
let decimals (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ".ctor", [Value(NewArray(ArrayValues ([low; mid; high; signExp] as args),_,_),_)] ->
        Helper.LibCall(com, "Decimal", "fromInts", t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
    | ".ctor", [arg] ->
        match arg.Type with
        | Array (Number(Int32, NumberInfo.Empty),_) ->
            Helper.LibCall(com, "Decimal", "fromIntArray", t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
...
let bigints (com: ICompiler) (ctx: Context) r (t: Type) (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | callee, meth ->
        let args =
            ...
            | None, _ -> args
            | Some c, _ -> c::args
        Helper.LibCall(com, "BigInt", Naming.lowerFirst meth, t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
...
let intrinsicFunctions (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | ("GetArraySlice" | "GetStringSlice" as meth), None, [ar; lower; upper] ->
        ...
        let meth = if meth = "GetStringSlice" then "substring" else "sublist"
...
//    | "get_IsReadOnly", _ -> makeBoolConst false |> Some
    ...
    | "TryGetValue", _, _ ->
       Helper.LibCall(com, "Types", "tryGetValue", t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?thisArg=thisArg, ?loc=r) |> Some
    | "Add", Some c, _ ->
...
let bitConvert (com: ICompiler) (ctx: Context) r t (i: CallInfo) (_: Expr option) (args: Expr list) =
    ...
    | "GetBytes" ->
        ...
        Helper.LibCall(com, "BitConverter", memberName, Boolean, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
    | _ ->
...
//        round com args |> toInt com ctx r t |> Some
    ...
    | "ToBase64String" | "FromBase64String" ->
        if not(List.isSingle args) then
            ...
            |> addWarning com ctx.InlinePath r
        Helper.LibCall(com, "String", (Naming.lowerFirst i.CompiledName), t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
    | _ -> None
...
let timers (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | meth, Some x, args -> Helper.InstanceCall(x, meth, t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
    | _ -> None
...
let regex com (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    ...
    | "get_Index", Some thisArg ->
        if isGroup thisArg then
            ...
            |> addErrorAndReturnNull com ctx.InlinePath r |> Some
        else getFieldWith r t thisArg "start" |> Some
...
let comparables (com: ICompiler) (ctx: Context) r t (i: CallInfo) (thisArg: Expr option) (args: Expr list) =
    match thisArg, i.CompiledName with
    | Some callee, "CompareTo" -> Helper.InstanceCall(callee, "compareTo", t, args, i.SignatureArgTypes, ?loc=r) |> Some
    | _ -> None
...
let fsharpType com methName (r: SourceLocation option) t (i: CallInfo) (args: Expr list) =
    ...
    | "IsUnion" | "IsRecord" | "IsTuple" | "IsFunction" ->
        Helper.LibCall(com, "Reflection", Naming.lowerFirst methName, t, args, i.SignatureArgTypes, genArgs=i.GenericArgs, ?loc=r) |> Some
