...
let getTypeReflectionMethodsForFields (com: IPhpCompiler) (fields: Fable.Field list) =
    [ for field in fields do
                  { PhpFun.Name = sprintf "get_%s_Type" field.Name
                    ...
                    PhpFun.Static = true
                    PhpFun.Body =
                        [ PhpStatement.PhpReturn(convertTypeToPhp com field.FieldType)]
                  } ]
              ...
              { PhpFun.Name = "allCases";
                ...
                PhpFun.Body =
                    [ PhpStatement.PhpReturn(
                        PhpNewArray([
                            PhpArrayNoIndex, PhpConst (PhpConstString (sprintf "\%s\%s" com.PhpNamespace case.Name))
                        ]))
                    ]
                }
                  ...
                  { PhpFun.Name = "allCases";
                    ...
                    PhpFun.Body = 
                        [ PhpStatement.PhpReturn(
                            PhpNewArray(
                                info.UnionCases
                                |> Seq.map (fun case -> (PhpArrayNoIndex, PhpConst(PhpConstString(caseNameWithNamespace com info case))))
                                |> List.ofSeq
                            ))]
                    }
              ]
