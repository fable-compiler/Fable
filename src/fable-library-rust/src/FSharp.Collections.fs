...
module HashIdentity =
    ...
    let Structural<'T when 'T: equality> : IEqualityComparer<'T> =
        EqualityComparer<'T>.Create(LanguagePrimitives.GenericEquality, LanguagePrimitives.GenericHash)

    let Reference<'T when 'T: not struct> : IEqualityComparer<'T> =
        EqualityComparer<'T>.Create(LanguagePrimitives.PhysicalEquality, LanguagePrimitives.PhysicalHash)
