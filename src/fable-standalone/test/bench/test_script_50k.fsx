...
module RayTrace1 =
    ...
    type Color(r: float, g: float, b: float) =
        ...
        static member ToDrawingColor(c: Color) =
            new Color(clamp c.R, clamp c.G, clamp c.B)
    ...
    type RayTracer(screenWidth, screenHeight) =
        ...
        let GetPoint x y (camera: Camera) =
            let RecenterX x =
                (float x - (float screenWidth / 2.0))
                / (2.0 * float screenWidth)

            let RecenterY y =
                -(float y - (float screenHeight / 2.0))
                / (2.0 * float screenHeight)
        ...
        let Checkerboard =
            { new Surface with
                member s.Diffuse pos =
                    ...
                    if ((int pos.Z) + (int pos.X)) % 2 <> 0 then
                        Color.White
                    else
                        Color.Black
