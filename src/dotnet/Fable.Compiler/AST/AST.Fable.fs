...
type Expr =
    ...
    member this.SubExpressions: Expr list =
        match this with
        ...
        | ObjectExpr(members,_, baseCall) ->
            let baseCall = Option.toList baseCall
        ...
        | Operation(kind, _, _) ->
            ...
            | Call(kind, info) ->
                ...
                let thisArg = Option.toList info.ThisArg
                kind @ thisArg @ info.Args
            | Emit(_, info) ->
                match info with
                | Some info -> (Option.toList info.ThisArg) @ info.Args
                | None -> []
        ...
        | TryCatch(body, catch, finalizer) ->
            body :: (Option.map snd catch |> Option.toList) @ (Option.toList finalizer)
        | DecisionTree(expr, targets) ->
            let targets = targets |> List.map (fun (_, v) -> v)
            expr::targets
