...
type Program(fileName, loc, body, ?directives, ?logs, ?dependencies, ?sourceFiles) =
    ...
    member x.dependencies: string[] = defaultArg dependencies [||]
    /// List of sources for .fsproj files
    member x.sourceFiles: string[] = defaultArg sourceFiles [||]
