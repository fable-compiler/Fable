...
module Patterns =
    ...
    let typeEquals (s:Type) (t:Type) = s = t
    ...
    let bindMethodBySearch (parentT:Type, nm, marity, argtys, rty) =
        let methInfos = parentT.GetMethods() |> Array.toList
    ...
    let bindModuleFunction (ty:Type, nm) =
        match ty.GetMethods() |> Array.filter (fun mi -> mi.Name = nm) with
                        ...
                        let weight (mi : MethodInfo) =
                            ...
                            let rec iter i acc =
                                ...
                                if param.ParameterType.IsGenericParameter then
                                    let actualTy = typeof<unit> // TODO: find parameter //tyArgs.[param.ParameterType.GenericParameterPosition]
...
#else
        typ.GetProperty(propName) |> checkNonNullResult ("propName", "String.Format(SR.GetString(SR.QfailedToBindProperty), propName)") // fxcop may not see "propName" as an arg
#endif
    let bindField (tc, fldName, tyargs) =
        let typ = mkNamedType (tc, tyargs)
        typ.GetField(fldName) |> checkNonNullResult ("fldName", "String.Format(SR.GetString(SR.QfailedToBindField), fldName)") // fxcop may not see "fldName" as an arg
    ...
    let bindGenericCtor (tc:Type, argTypes:Instantiable<Type list>) =
        ...
        |> checkNonNullResult ("tc", "SR.GetString(SR.QfailedToBindConstructor)")
#else
        tc.GetConstructor(Array.ofList argtyps) |> checkNonNullResult ("tc", "SR.GetString(SR.QfailedToBindConstructor)")
#endif
    ...
    let bindCtor (tc, argTypes:Instantiable<Type list>, tyargs) =
        ...
        |> checkNonNullResult ("tc", "SR.GetString(SR.QfailedToBindConstructor)")
#else
        typ.GetConstructor(Array.ofList argtyps) |> checkNonNullResult ("tc", "SR.GetString(SR.QfailedToBindConstructor)")
#endif
    ...
    module SimpleUnpickle =
        ...
        let unpickleObj referencedTypeDefs u phase2bytes =
            let phase2data =
                let st2 =
...
//             | assembly -> assembly
    ...
    let u_NamedType st =
        ...
        | _ ->
            ...
            failwith "mkNamedTycon (escapedTcName, assemblyRef)"
    ...
    and u_VarDecl st =
        ...
        (fun env -> Var(s, b env.typeInst, mut))
    ...
    let unpickleExpr (localType: Type) referencedTypes bytes =
        unpickleObj referencedTypes u_Expr bytes

    let unpickleReflectedDefns referencedTypes bytes =
        unpickleObj referencedTypes u_ReflectedDefinitions bytes
    ...
    type ReflectedDefinitionTableKey =
        ...
        static member GetKey(methodBase:MethodBase) =
            // TODO: better key
            Key (methodBase.DeclaringType.FullName + "." + methodBase.Name)
    ...
    let registerReflectedDefinitions (resourceName, bytes, referencedTypes) =
        let defns = unpickleReflectedDefns referencedTypes bytes
        ...
        decodedTopResources.Add((resourceName), 0)
...
//                             yield (resourceName, unpickleReflectedDefns referencedTypes resourceBytes) ]
...
type Expr with
    ...
    static member RegisterReflectedDefinitions(resource, serializedValue, referencedTypes) =
