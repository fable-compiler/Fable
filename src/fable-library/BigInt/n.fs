...
module internal BigNatModule =
    ...
    module FFT =
        ...
        let leastBounding2Power b =
            let rec findBounding2Power b tp i =
                if b <= tp then
                    tp, i
                else
                    findBounding2Power b (tp * 2) (i + 1) in
        ...
        let computeInverseFftInPlace n w uT =
            ...
            Array.map (mmul bigKInv) (computFftInPlace n (minv w) uT)
        ...
        let rec computeFftReference n w u =
            ...
            if n = 1 then
                ...
                Array.init
                    ...
                    (fun j ->
                        if j < n / 2 then
                            madd (uevenFT.[j]) (mmul (mpow w j) (uoddFT.[j]))
                        else
                            let j = j - (n / 2)
                            msub (uevenFT.[j]) (mmul (mpow w j) (uoddFT.[j]))
                    )
#endif
    ...
    let copyN x =
        {
            bound = x.bound
            digits = Array.copy x.digits
        } // could copy just enough...

    let normN n =
        // normalises bound
        let rec findLeastBound (na: ints) i =
            if i = -1 || na.[i] <> 0 then
                i + 1
            else
                findLeastBound na (i - 1)
    ...
    let shiftCompare p pn q qn =
        if p.bound + pn < q.bound + qn then
            -1
        elif p.bound + pn > q.bound + pn then
            1
        else
            let rec check (pa: ints) (qa: ints) i =
    ...
    let compare p q =
        if p.bound < q.bound then
            -1
        elif p.bound > q.bound then
            1
        else
            let rec check (pa: ints) (qa: ints) i =
                // HAVE: pa.[j] = qa.[j] for i < j < p.bound
...
*)
    ...
    let mulSchoolBook p q =
        ...
        if (pSmall && qSmall) then
            mulSchoolBookBothSmall (coeff p 0) (coeff q 0)
        elif pSmall then
            mulSchoolBookOneSmall q (coeff p 0)
        elif qSmall then
            mulSchoolBookOneSmall p (coeff q 0)
        else
            mulSchoolBookNeitherSmall p q
    ...
    let wordBits word =
        let rec hi k =
            if k = 0 then
                0
            elif (word &&& twopowers.[k - 1]) <> 0 then
                k
            else
                hi (k - 1)
    ...
    let bits u =
        if u.bound = 0 then
            ...
            degree u * baseBits + wordBits u.digits.[degree u]
...
*)
   ...
   *)
    ...
    let removeFactor x a n =
        ...
        if degx < dega + n then
            0
        else // possible with "normalisation"
    ...
    let divmod b a =
        ...
        else if degree b < degree a then
    ...
    let bitAnd a b =
        let rbound = minInt a.bound b.bound
        let r = createN rbound

        for i = 0 to r.bound - 1 do
    ...
    let hcf a b =
        ...
        if lt a b then
            hcfloop a b // Have: 0 <= a <  b
        else
            hcfloop b a // Have: 0 <= b <= a
    ...
    let powi x n =
        let rec power acc x n =
            if n = 0 then
                ...
                power acc (mul x x) (n / 2)
            else
                power (mul x acc) (mul x x) (n / 2)
    ...
    let pow x n =
        let rec power acc x n =
            if isZero n then
                ...
                if isZero nmod2 then
                    power acc (mul x x) ndiv2
                else
                    power (mul x acc) (mul x x) ndiv2
    ...
    let toString n =
        ...
        let rec collect isLeading digits n (arg: list<int * BigNat>) =
            match arg with
#else
        let rec collect isLeading digits n =
            function
#endif
            | [] ->
...
#if CHECKED
                if checks then
                    check (0 <= n) "toString: digit0"

                if checks then
                    check (n <= 9) "toString: digit9"
#endif
                if isLeading && n = 0 then
                    digits // suppress leading 0
                else
                    string n :: digits
            | (_, ten2k) :: prior ->
#if CHECKED
                if checks then
                    check (lt n (mul ten2k ten2k)) "string_of_int: bound n"
#endif
                // Have 0 <= n     < (ten2k)^2 and ten2k = 10^(2^k)
                let nH, nL = divmod n ten2k
#if CHECKED
                if checks then
                    check (lt nH ten2k) "string_of_int: bound nH"

                if checks then
                    check (lt nL ten2k) "string_of_int: bound nL"
#endif
