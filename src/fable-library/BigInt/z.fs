...
type BigInteger(signInt: int, v: BigNat) =
    ...
    static member internal nat n =
        if BigNatModule.isSmall n && BigNatModule.getSmall n < smallLim then
            smallPosTab.[BigNatModule.getSmall n]
        else
            n
    ...
    static member internal compare(n, nn) =
        if BigInteger.op_LessThan (n, nn) then
            -1
        elif BigInteger.op_Equality (n, nn) then
            0
        else
            1
    ...
    static member internal hash(z: BigInteger) =
        if z.SignInt = 0 then
            1 // 1 is hashcode for initialized BigInteger.Zero
        else
            z.SignInt + BigNatModule.hash (z.V)

    override x.ToString() =
        match x.SignInt with
        | 1 -> BigNatModule.toString x.V // positive
        | -1 ->
            if BigNatModule.isZero x.V then
                "0" // not negative in fact, but zero.
            else
                "-" + BigNatModule.toString x.V // negative
        | 0 -> "0"
    ...
    interface Fable.Core.IJsonSerializable with
        member this.toJSON() = this.ToString() |> box
    ...
    new(n: int32) =
        if n >= 0 then
            ...
            BigInteger(-1, BigInteger.nat (BigNatModule.ofInt64 (-(int64 n))))
        else
            BigInteger(-1, BigInteger.nat (BigNatModule.ofInt32 (-n)))
    ...
    static member Scale(k, z: BigInteger) =
        if z.SignInt = 0 then
            ...
            BigInteger.create (-z.SignInt, (BigNatModule.scale (-k) z.V)) // k.zsign.zv = -zsign.(-k.zv)
        else
            BigInteger.create (z.SignInt, (BigNatModule.scale k z.V)) // k.zsign.zv =  zsign.k.zv

    // Result: 1.nx - 1.ny  (integer subtraction)
    static member internal subnn(nx, ny) =
        if BigNatModule.gte nx ny then
            BigInteger.posn (BigNatModule.sub nx ny) // nx >= ny, result +ve,  +1.(nx - ny)
        else
            BigInteger.negn (BigNatModule.sub ny nx) // nx < ny,  result -ve,  -1.(ny - nx)
    ...
    static member Abs(x: BigInteger) =
        if x.SignInt = -1 then
            -x
        else
            x
    ...
    static member Pow(x: BigInteger, y: int32) =
        if y < 0 then
            raise (new System.ArgumentOutOfRangeException("y")) //, (SR.GetString(SR.inputMustBeNonNegative))))
    ...
    static member Parse(text: string) =
        if isNull text then
            raise (new ArgumentNullException("text"))
    ...
    static member Factorial(x: BigInteger) =
        if x.IsNegative then
            invalidArg "x" "mustBeNonNegative" //(SR.GetString(SR.inputMustBeNonNegative))

        if x.IsPositive then
            BigInteger.posn (BigNatModule.factorial x.V)
        else
            BigInteger.One
