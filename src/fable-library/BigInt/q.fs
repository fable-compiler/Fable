...
type BigRationalLarge =
    ...
    static member Hash(Q(ap, aq)) =
        ...
        if aq.IsOne then
            ...
            (ap.GetHashCode() <<< 3) + aq.GetHashCode()
    ...
    static member Compare(p, q) =
        if BigRationalLarge.LessThan(p, q) then
            -1
        elif BigRationalLarge.LessThan(q, p) then
            1
        else
            0
    ...
    static member ToDouble(Q(p, q)) = ToDoubleI p / ToDoubleI q
...
module BigRationalLarge =
    ...
    let pown (Q(p, q)) (n: int) =
        Q(BigInteger.Pow(p, n), BigInteger.Pow(q, n)) // p,q powers still coprime
...
type BigRational =
    ...
    interface System.IComparable with
        member n1.CompareTo(obj: obj) =
            match obj with
            | :? BigRational as n2 ->
                if BigRational.(<) (n1, n2) then
                    -1
                elif BigRational.(=) (n1, n2) then
                    0
                else
                    1
...
module NumericLiteralN =
    ...
    let FromInt64 (i64: int64) =
        BigRational.FromBigInt(new BigInteger(i64))
