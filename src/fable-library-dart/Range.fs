...
let integralRangeStep<'T> (start: 'T) (step: 'T) (stop: 'T) (zero:'T) (add: 'T -> 'T -> 'T) (compare:'T -> 'T -> int) =
    let stepFn = makeRangeStepFunction step stop zero add compare
    Seq.delay(fun () -> Seq.unfold stepFn start)

let rangeDouble start step stop = integralRangeStep start step stop 0.0 (+) compare
let rangeInt start step stop = integralRangeStep start step stop 0 (+) compare
