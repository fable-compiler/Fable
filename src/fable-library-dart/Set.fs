...
module SetTree =
    ...
    let rec foldBackOpt (f: 'T -> 'a -> 'a) (t: SetTree<'T>) (x: 'a) : 'a =
        match t with
        | None -> x
        | Some t2 ->
            match t2 with
            | :? SetTreeNode<'T> as tn -> foldBackOpt f tn.Left (f tn.Key (foldBackOpt f tn.Right x))
            | _ -> f t2.Key x

    let foldBack (f: 'a -> 'b -> 'b) (m: SetTree<'a>) (x: 'b) : 'b = foldBackOpt f m x

    let rec foldOpt (f: 'a -> 'T -> 'a) (x: 'a) (t: SetTree<'T>) : 'a =
        match t with
        | None -> x
        | Some t2 ->
            match t2 with
    ...
    let fold (f: 'a -> 'b -> 'a) (x: 'a) (m: SetTree<'b>) : 'a = foldOpt f x m
    ...
    let rec moveNext i =
        if i.started then
            match i.stack with
            | [] -> false
            | None :: _rest -> failwith "Please report error: Set iterator, unexpected stack for moveNext"
            | Some t :: rest ->
                match t with
    ...
    let mkIEnumerator s =
        new SetEnumerator<_>(s) :> IEnumerator<_>
...
type Set<[<EqualityConditionalOn>] 'T when 'T: comparison>(comparer: IComparer<'T>, tree: SetTree<'T>) =
    ...
    interface System.IComparable<Set<'T>> with
        member s.CompareTo(that: Set<'T>) =
            SetTree.compare s.Comparer s.Tree that.Tree
