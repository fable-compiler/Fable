...
// #endif
    ...
    let partition1 (comparer: IComparer<'Key>) (f: 'Key -> 'a -> bool) (k: 'Key) (v: 'a) ((acc1, acc2): MapTree<'Key,'a> * MapTree<'Key,'a>): MapTree<'Key,'a> * MapTree<'Key,'a> =
        if f k v then (add comparer k v acc1, acc2) else (acc1, add comparer k v acc2)

    let rec partitionAux (comparer: IComparer<'Key>) (f: 'Key -> 'Value -> bool) (m: MapTree<'Key, 'Value>) (acc: MapTree<'Key,'Value> * MapTree<'Key,'Value>): MapTree<'Key,'Value> * MapTree<'Key,'Value> =
        match m with
        | None -> acc
        | Some m2 ->
            match m2 with
    ...
    let partition (comparer: IComparer<'Key>) (f: 'Key -> 'a -> bool) (m: MapTree<'Key,'a>): MapTree<'Key,'a> * MapTree<'Key,'a> =
        partitionAux comparer f m (empty, empty)

    let filter1 (comparer: IComparer<'Key>) (f: 'Key -> 'a -> bool) (k: 'Key) (v: 'a) (acc: MapTree<'Key,'a>): MapTree<'Key,'a> =
        if f k v then add comparer k v acc else acc

    let rec filterAux (comparer: IComparer<'Key>) (f: 'Key -> 'Value -> bool) (m: MapTree<'Key, 'Value>) (acc: MapTree<'Key,'Value>): MapTree<'Key,'Value> =
        match m with
        | None -> acc
        | Some m2 ->
            match m2 with
    ...
    let filter (comparer: IComparer<'Key>) (f: 'Key -> 'a -> bool) (m: MapTree<'Key,'a>): MapTree<'Key,'a> =
        filterAux comparer f m empty

    let rec spliceOutSuccessor (m: MapTree<'Key, 'Value>): 'Key * 'Value * MapTree<'Key,'Value> =
        match m with
    ...
    let rec remove (comparer: IComparer<'Key>) (k: 'Key) (m: MapTree<'Key, 'Value>): MapTree<'Key,'Value> =
        match m with
        | None -> empty
        | Some m2 ->
            let c = comparer.Compare(k, m2.Key)
    ...
    let rec change (comparer: IComparer<'Key>) (k: 'Key) (u: 'Value option -> 'Value option) (m: MapTree<'Key, 'Value>) : MapTree<'Key,'Value> =
        match m with
        | None ->
            match u None with
            | None -> m
    ...
    let rec mem (comparer: IComparer<'Key>) (k: 'Key) (m: MapTree<'Key, 'Value>): bool =
        match m with
        | None -> false
        | Some m2 ->
            let c = comparer.Compare(k, m2.Key)
    ...
    let rec iterOpt (f: 'Key -> 'Value -> unit) (m: MapTree<'Key, 'Value>): unit =
        match m with
        | None -> ()
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn -> iterOpt f mn.Left; f mn.Key mn.Value; iterOpt f mn.Right
            | _ -> f m2.Key m2.Value

    let iter (f: 'a -> 'b -> unit) (m: MapTree<'a,'b>): unit =
        iterOpt f m

    let rec tryPickOpt (f: 'Key -> 'Value -> 'a option) (m: MapTree<'Key, 'Value>): 'a option =
        match m with
        | None -> None
        | Some m2 ->
            match m2 with
    ...
    let rec existsOpt (f: 'Key -> 'Value -> bool) (m: MapTree<'Key, 'Value>): bool =
        match m with
        | None -> false
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn -> existsOpt f mn.Left || f mn.Key mn.Value || existsOpt f mn.Right
            | _ -> f m2.Key m2.Value

    let exists (f: 'a -> 'b -> bool) (m: MapTree<'a,'b>): bool =
        existsOpt f m

    let rec forallOpt (f: 'Key -> 'Value -> bool) (m: MapTree<'Key, 'Value>): bool =
        match m with
        | None -> true
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn -> forallOpt f mn.Left && f mn.Key mn.Value && forallOpt f mn.Right
            | _ -> f m2.Key m2.Value

    let forall (f: 'a -> 'b -> bool) (m: MapTree<'a,'b>): bool =
        forallOpt f m
    ...
    let rec mapiOpt (f: 'Key -> 'Value -> 'Result) (m: MapTree<'Key, 'Value>): MapTree<'Key,'Result> =
        match m with
        | None -> empty
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn ->
                ...
                let v2 = f mn.Key mn.Value
                let r2 = mapiOpt f mn.Right
                MapTreeNode (mn.Key, v2, l2, r2, mn.Height) :> MapTreeLeaf<'Key, 'Result> |> Some
            | _ -> MapTreeLeaf (m2.Key, f m2.Key m2.Value) |> Some

    let mapi (f: 'a -> 'b -> 'c) (m: MapTree<'a,'b>): MapTree<'a,'c> =
        mapiOpt f m

    let rec foldBackOpt (f: 'Key -> 'Value -> 'a -> 'a) (m: MapTree<'Key, 'Value>) (x: 'a): 'a =
        match m with
        | None -> x
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn ->
                ...
                let x = f mn.Key mn.Value x
                foldBackOpt f mn.Left x
            | _ -> f m2.Key m2.Value x

    let foldBack (f: 'a -> 'b -> 'c -> 'c) (m: MapTree<'a,'b>) (x: 'c): 'c =
        foldBackOpt f m x

    let rec foldOpt (f: 'a -> 'Key -> 'Value -> 'a) (x: 'a) (m: MapTree<'Key, 'Value>): 'a =
        match m with
        | None -> x
        | Some m2 ->
            match m2 with
            | :? MapTreeNode<'Key, 'Value> as mn ->
                ...
                let x = f x mn.Key mn.Value
                foldOpt f x mn.Right
            | _ -> f x m2.Key m2.Value

    let fold (f: 'a -> 'b -> 'c -> 'a) (x: 'a) (m: MapTree<'b,'c>): 'a =
        foldOpt f x m

    let foldSectionOpt (comparer: IComparer<'Key>) (lo: 'Key) (hi: 'Key) (f: 'Key -> 'Value -> 'a -> 'a) (m: MapTree<'Key, 'Value>) (x: 'a): 'a =
        let rec foldFromTo (f: 'Key -> 'Value -> 'b -> 'b) (m: MapTree<'Key, 'Value>) (x: 'b): 'b =
            match m with
            | None -> x
            | Some m2 ->
                match m2 with
                | :? MapTreeNode<'Key, 'Value> as mn ->
                    ...
                    let x = if cLoKey <= 0 && cKeyHi <= 0 then f mn.Key mn.Value x else x
                ...
                | _ ->
                    ...
                    let x = if cLoKey <= 0 && cKeyHi <= 0 then f m2.Key m2.Value x else x
                    x

        if comparer.Compare(lo, hi) = 1 then x else foldFromTo f m x

    let foldSection (comparer: IComparer<'Key>) (lo: 'Key) (hi: 'Key) (f: 'Key -> 'a -> 'b -> 'b) (m: MapTree<'Key,'a>) (x: 'b): 'b =
        foldSectionOpt comparer lo hi f m x

    let toList (m: MapTree<'Key, 'Value>): ('Key * 'Value) list =
    ...
    let copyToArray (m: MapTree<'a,'b>) (arr: _[]) (i: int): unit =
        let mutable j = i
        iter (fun x y -> arr[j] <- KeyValuePair(x, y); j <- j + 1) m

    let ofList (comparer: IComparer<'a>) (l: ('a * 'b) list): MapTree<'a,'b> =
        List.fold (fun acc (k, v) -> add comparer k v acc) empty l

    let rec mkFromEnumerator (comparer: IComparer<'a>) (acc: MapTree<'a,'b>) (e : IEnumerator<_>): MapTree<'a,'b> =
        if e.MoveNext() then
            let (x, y) = e.Current
            mkFromEnumerator comparer (add comparer x y acc) e
        else acc

    let ofArray (comparer: IComparer<'Key>) (arr: array<'Key * 'Value>): MapTree<'Key,'Value> =
        let mutable res = empty
        for (x, y) in arr do
            res <- add comparer x y res
        res

    let ofSeq (comparer: IComparer<'Key>) (c: seq<'Key * 'Value>): MapTree<'Key,'Value> =
        match c with
    ...
    let rec collapseLHS (stack: MapTree<'Key, 'Value> list): MapTree<'Key,'Value> list =
        match stack with
        | [] -> []
        | m :: rest ->
            match m with
    ...
    type MapEnumerator<'a, 'b when 'a : comparison>(m) =
        let mutable i = mkIterator m
        interface IEnumerator<KeyValuePair<'a,'b>> with
    ...
    let mkIEnumerator m = new MapEnumerator<_,_>(m) :> IEnumerator<_>
    ...
    let toArray (m: MapTree<'a,'b>): KeyValuePair<'a,'b>[] =
        let n = size m
        let e = mkIEnumerator m
        ArrayModule.Native.generate n (fun _ ->
            e.MoveNext() |> ignore
            e.Current)
...
// #endif
    ...
    interface System.IComparable<Map<'Key, 'Value>> with
        member m.CompareTo(m2: Map<'Key, 'Value>) =
            Seq.compareWith
...
//        member m.forEach(f, ?thisArg) = m |> Seq.iter (fun p -> f p.Value p.Key m)
    ...
    override this.ToString() =
        let inline toStr (kv: KeyValuePair<'Key,'Value>) = $"({kv.Key}, {kv.Value})"
...
let isEmpty (table: Map<_, _>): bool =
    table.IsEmpty

// [<CompiledName("Add")>]
let add (key: 'a) (value: 'b) (table: Map<_, _>): Map<'a,'b> =
    table.Add (key, value)

// [<CompiledName("Change")>]
let change (key: 'a) (f: 'b option -> 'b option) (table: Map<_, _>): Map<'a,'b> =
    table.Change (key, f)

// [<CompiledName("Find")>]
let find (key: 'a) (table: Map<_, _>): 'b =
    table[key]

// [<CompiledName("TryFind")>]
let tryFind (key: 'a) (table: Map<_, _>): 'b option =
    table.TryFind key
