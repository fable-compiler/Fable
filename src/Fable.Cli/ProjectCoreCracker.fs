...
let private projInfo additionalMSBuildProps (file: string) =
    ...
    let toolsPath = Ionide.ProjInfo.Init.init (DirectoryInfo projDir) None
    let loader = Ionide.ProjInfo.WorkspaceLoader.Create(toolsPath, additionalMSBuildProps)
    let proj = loader.LoadProjects ([file], ["IsCrossTargetingBuild"], Ionide.ProjInfo.BinaryLogGeneration.Off) |> Seq.head
    ...
    let proj =
        if isCrossTargeting then
            ...
            | (first :: _) ->
                ...
                let loader = Ionide.ProjInfo.WorkspaceLoader.Create(toolsPath, ("TargetFramework", first) :: additionalMSBuildProps)
                loader.LoadProjects ([file], [], Ionide.ProjInfo.BinaryLogGeneration.Off) |> Seq.head
        else
            proj
