...
%% Convert a numeric value to a binary of bytes (big-endian, matching BEAM convention).
...
get_bytes(Value, BitSize) when is_integer(Value) ->
    <<Value:BitSize/big-signed-integer>>;
...
to_int(Bytes, StartIndex, BitSize) when is_binary(Bytes) ->
    <<_:StartIndex/binary, Value:BitSize/big-signed-integer, _/binary>> = Bytes,
    Value;
...
to_uint(Bytes, StartIndex, BitSize) when is_binary(Bytes) ->
    <<_:StartIndex/binary, Value:BitSize/big-unsigned-integer, _/binary>> = Bytes,
    Value;
...
to_float(Bytes, StartIndex, BitSize) when is_binary(Bytes) ->
    <<_:StartIndex/binary, Value:BitSize/big-float, _/binary>> = Bytes,
    Value;
