...
function isCall($e) {
    ...
    // A static/operator call carries the "novalue" sentinel node as its instance
    // (distinct from a genuine quoted null, tagged "null"); expose it as null so
    // Patterns.Call matches F#.
    $inst = $e['instance'];
    if (is_array($inst) && ($inst['tag'] ?? null) === 'Value' && ($inst['type'] ?? null) === 'novalue') {
        $inst = null;
    }
...
function isFieldGet($e) {
    if ($e['tag'] !== 'FieldGet') return null;
    // A static property getter carries the "novalue" sentinel node as its instance
    // (distinct from a genuine quoted null, tagged "null"); expose it as null so
    // Patterns.PropertyGet matches F#. Third slot mirrors PropertyGet's indexer-args,
    // always empty here (field/property accessors are never indexed).
    $inst = $e['expr'];
    if (is_array($inst) && ($inst['tag'] ?? null) === 'Value' && ($inst['type'] ?? null) === 'novalue') {
        $inst = null;
    }
    return [$inst, $e['fieldName'], []];
}
