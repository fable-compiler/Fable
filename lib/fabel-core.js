...
(function (global, factory) {
  ...
  FString.fsFormatRegExp = /%[+\-* ]?\d*(?:\.(\d+))?(\w)/g;

  FString.fsFormat = function (str, args, reg) {
    var i = 0;
    reg = reg || FString.fsFormatRegExp;
    return str.replace(reg, function (_, precision, format) {
      var rep = args[i++];
      switch (format) {
  ...
  FString.fsFormatCurried = function (str) {
    // Build new RegExp without 'g' flag
    var reg = new RegExp(FString.fsFormatRegExp.source);
    function makeFn(str) {
      return function (rep) {
        var tmp = FString.fsFormat(str, [rep], reg);
        return reg.test(tmp) ? makeFn(tmp) : tmp;
      }
    }
    return makeFn(str);
  };
  ...
  FString.split = function (str, splitters, removeEmpty) {
    splitters = ArrayBuffer.isView(splitters) ? Array.from(splitters) : splitters;
    splitters = Array.isArray(splitters) ? splitters : Util.getRestParams(arguments, 1);
    var reg = new RegExp("[" + splitters.map(function (x) {
      return Regex.escape(typeof x === "number" ? String.fromCharCode(x) : x);
    }).join("") + "]", "g");
  ...
  FString.toCharArray = function (str) {
  ...
  FString.endsWith = function (str, search) {
    return (new RegExp(Regex.escape(search) + "$")).test(str);
  };  
