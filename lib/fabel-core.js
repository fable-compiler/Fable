...
(function (global, factory) {
  ...
  String.printFormatToString = function (str, args) {
  ...
  List.prototype.getSlice = function (lower, upper) {
  ...
  Seq.partition = function (xs) {
    throw "TODO";
  };
  Seq.permute = function (xs) {
    throw "TODO";
  };  
  ...
  Seq.sortBy = function (f, xs) {
    var ys = Seq.toArray(xs);
    return Seq.ofArray(Seq.sortInPlaceBy(f, ys));
  };
  ...
  Seq.sortByDescending = function (f, xs) {
    var ys = Seq.toArray(xs);
    return Seq.ofArray(Seq.sortInPlaceBy(f, ys, -1));
  };
  // Can only be applied to arrays
  Seq.sortInPlaceBy = function (f, xs, dir) {
    dir = dir || 1;
