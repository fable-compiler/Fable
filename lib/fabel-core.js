...
(function (global, factory) {
  ...
  F.Seq = (function(){
    ...
    Seq.foldi = function(f, acc, xs) {
      if (Array.isArray(xs)) {
        // Swap the order of index argument
        xs.reduce(function(acc,x,i){ return f(i,acc,x) }, acc);
      }
      else {
    ...
    Seq.foldBack = function(f, acc, xs) {
      if (Array.isArray(xs)) {
        xs.reduceRight(f, acc);
      }
      else {
        // TODO
      }
    };
    Seq.foldi2 = function(f, acc, xs) {
      // TODO
    };
