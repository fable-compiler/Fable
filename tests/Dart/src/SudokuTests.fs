module Fable.Tests.Dart.Sudoku
...
let cols (sudoku:Sudoku) =
    sudoku
    |> Array.mapi (fun a row -> row |> Array.mapi (fun b _cell -> sudoku[b][a]))
...
let boxes (sudoku:Sudoku) =
    ...
    let list = List<_>()
    for a in 0..(d*d) - 1 do list.Add(List<_>())
...
let tests() =
    ...
    testCase "wrong solution doesn't match" <| fun () ->
        ...
        wrongSolution = solution |> equal false
