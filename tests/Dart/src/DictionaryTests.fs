module Fable.Tests.Dart.Dictionary
...
let tests() =
    ...
    testCase "Interface IDictionary creation works" <| fun () ->
        let dic = dict <| seq { for i in 1. .. 10. -> i, i*i }
        equal 4. dic.[2]

    testCase "Dictionary creation from IDictionary works" <| fun () ->
        let idic = dict <| seq { for i in 1 .. 10 -> i, i*i }
    ...
    testCase "Dictionaries with IEqualityComparer work" <| fun () ->
        ...
        let dic2 = Dictionary<_,_>(MyRefTypeComparer())
        dic2.Add(x, "bar")
