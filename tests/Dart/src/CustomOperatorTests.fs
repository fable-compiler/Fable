module Fable.Tests.Dart.CustomOperator
...
let typeOperators() =
    ...
    testCase "Custom operator overloads work" <| fun () ->
        let x = { value = 5 }
        x + 2 |> equal { value = 7 }
        3 + x |> equal 10
...
let moduleOperators() =
    testCase "Custom operator overloads can be inlined" <| fun () ->
        let x = { value = 5 }
...
type ToLength = ToLength with
    static member inline (&.) (ToLength, x: int<px>) = fun D1 _ _ -> String.Join("", string x, "px")
    static member inline (&.) (ToLength, x: int<em>) = fun D1 D2 _ -> String.Join("", string x, "em")
...
let tests() =
  typeOperators()
  moduleOperators()
  operatorsAsFunctions()
  operatorsForOverloads()
