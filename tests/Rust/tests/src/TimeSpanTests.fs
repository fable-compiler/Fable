...
[<Fact>]
let ``TimeSpan.FromMinutes works`` () =
...
[<Fact>]
let ``TimeSpan.FromSeconds works`` () =
...
[<Fact>]
let ``TimeSpan.FromMicroseconds works`` () =
