// Value-based reflection uses thread_local on std and OnceInit on no_std.
[<Fable.Core.Rust.OuterAttr("cfg", [| "feature = \"reflection\"" |])>]
...
// box_lrc/unbox_lrc coerce the same smart pointer rather than cloning the value,
// so a record boxed to obj and unboxed again is physically the same instance.
[<Fact>]
let ``Boxing a record to obj and unboxing preserves reference identity`` () =
    let r =
        {
            String = "a"
            Int = 1
