module Fable.Tests.TestApplicative
...
[<Fact>]
let ``test Inline code doesn't call many times function with side effects`` () = // See #1321
...
[<Fact>]
let ``test Local inline values work`` () =
...
[<Fact>]
let ``test Local inline lambdas work standalone`` () = // See #1234
...
// [<Fact>]
//let ``test Partial application of optimized closures works`` () =
//   let mutable m = 1
...
[<Fact>]
let ``test Trait calls work with tuples`` () =
...
[<Fact>]
let ``test Closures generated by casts work`` () = // See #1150
...
[<Fact>]
let ``test Uncurrying works for base constructors`` () = // See #1458
    let str = AddString()
    str.MakeString "foo" "bar" |> equal "foobar"

[<Fact>]
let ``test Uncurrying works for base constructors II`` () = // See #1459
...
[<Fact>]
let ``test Partial Applying caches side-effects`` () = // See #1836
    PseudoElmish.reset()
...
[<Fact>]
let ``test Partial Applying locally caches side-effects`` () = // See #1836
    let mutable doMapRunTimes = 0
    let mutable setStateAccumulated = ""
...
[<Fact>]
let ``test OptimizedClosures.FSharpFunc<_,_,_>.Adapt works`` () = // See #1904
