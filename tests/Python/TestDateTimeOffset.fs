...
[<Fact>]
let ``test DateTimeOffset.MinValue works in pattern match`` () =
...
[<Fact>]
let ``test DateTimeOffset Equality works`` () =
...
[<Fact>]
let ``test DateTimeOffset Inequality works`` () =
    let test (ms: float) expected =
        let dt1 = DateTimeOffset(2014, 10, 9, 13, 23, 30, 234, TimeSpan.Zero)
        let dt2 = dt1.AddMilliseconds(ms)
        dt1 <> dt2 |> equal expected
...
[<Fact>]
let ``test DateTimeOffset GreaterThan works`` () =
    let test (ms: float) expected =
        let dt1 = DateTimeOffset(2014, 10, 9, 13, 23, 30, 234, TimeSpan.Zero)
        let dt2 = dt1.AddMilliseconds(ms)
        dt1 > dt2 |> equal expected
...
[<Fact>]
let ``test DateTimeOffset LessThan works`` () =
    let test (ms: float) expected =
        let dt1 = DateTimeOffset(2014, 10, 9, 13, 23, 30, 234, TimeSpan.Zero)
        let dt2 = dt1.AddMilliseconds(ms)
        dt1 < dt2 |> equal expected
...
[<Fact>]
let ``test DateTimeOffset UtcDateTime - LocalDateTime relationship`` () =
