...
// --- Option property-getter accessor (OptionValue) deconstructs as PropertyGet, matching
// real F# quotations. NOTE: cannot currently be executed — there is no PHP test runner
// (`./build.sh test php` is not implemented) and no PHP interpreter in this environment;
// these tests are added for coverage once a runner exists. ---
...
[<Fact>]
let ``test Static property access inside a quotation is a PropertyGet with no instance`` () =
    // Guards against the "novalue" no-instance sentinel (used for static property getters)
    // leaking out as a raw node instead of being normalized to None.
    let q = <@ QuotTestConfig.Version @>
