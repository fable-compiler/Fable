...
[<Fact>]
let ``test Set plus operator works`` () =
    let xs = Set.singleton 1
    let ys = Set.singleton 2
    let zs = xs + ys
    (zs.Contains 1 && zs.Contains 2)
...
[<Fact>]
let ``test Set minus operator works`` () =
    let xs = set [1.; 2.; 3.; 4.]
    let ys = set [1.; 2.]
    let zs = xs - ys
    zs.Count |> equal 2
...
[<Fact>]
let ``test xs.IsSupersetOf works`` () =
    let xs = set [1.; 2.; 3.; 4.]
    let ys = set [1.; 2.]
    xs.IsSupersetOf ys
    |> equal true
...
[<Fact>]
let ``test xs.IsProperSubsetOf works`` () =
    let xs = set [1.; 2.; 3.; 4.]
    let ys = set [1.; 2.]
...
[<Fact>]
let ``test xs.IsProperSupersetOf works`` () =
    let xs = set [1.; 2.; 3.; 4.]
    let ys = set [1.; 2.]
...
[<Fact>]
let ``test Set deduplicates elements`` () =
    let xs = set [1; 1; 2; 2; 3]
    Set.count xs |> equal 3

[<Fact>]
let ``test Set.map re-sorts after mapping`` () =
    let xs = set [1; 2; 3]
