...
[<Fact>]
let ``test TryCreate from absolute string with kind Absolute works`` () =
...
[<Fact>]
let ``test TryCreate from absolute string with kind RelativeOrAbsolute works`` () =
...
[<Fact>]
let ``test TryCreate from absolute string with kind Relative fails`` () =
    let (valid, _) = Uri.TryCreate("http://www.test1.com/hello?a=b#c", UriKind.Relative)
    equal false valid

[<Fact>]
let ``test TryCreate from relative string with kind Absolute fails`` () =
    let (valid, _uri) = Uri.TryCreate("hello?a=b#c", UriKind.Absolute)
    equal false valid

[<Fact>]
let ``test TryCreate from relative string with kind RelativeOrAbsolute works`` () =
    let (valid, uri) = Uri.TryCreate("hello?a=b#c", UriKind.RelativeOrAbsolute)
    equal true valid
    equal "hello?a=b#c" (uri.ToString())

[<Fact>]
let ``test TryCreate from relative string with kind Relative works`` () =
    let (valid, uri) = Uri.TryCreate("hello?a=b#c", UriKind.Relative)
    equal true valid
    equal "hello?a=b#c" (uri.ToString())

[<Fact>]
let ``test TryCreate from absolute uri and absolute uri should work`` () =
...
[<Fact>]
let ``test TryCreate from absolute uri and relative uri should work`` () =
