...
// Skipped: sideEffectsCounter uses closure over mutable variable (not supported at module level)
...
[<Fact>]
let ``test Picks the right witness`` () =
...
[<Fact>]
let ``test Infix applicative with inline functions can be generated`` () =
    let r = Ok 1
    let a = Ok string
...
[<Fact>]
let ``test Infix applicative with inline composed functions can be generated`` () =
    let r = Ok 1
    let a = Ok (string >> int)
    match applyInline a r with
    | Ok x -> equal 1 x
...
[<Fact>]
let ``test Infix applicative with even more inline functions can be generated`` () =
    let r = Ok (fun x -> x + 1)
...
// Skipped: depends on sideEffectsCounter (module-level mutable closure)
...
[<Fact>]
let ``test Local inline typed lambdas work`` () =
...
[<Fact>]
let ``test Lens.get returns correct values`` () =
    Lens_get fst_ ("Good","Bad") =! "Good"

[<Fact>]
let ``test Lens.set sets value correctly`` () =
    Lens_set fst_ "Good" ("Bad",()) =! ("Good",())

[<Fact>]
let ``test Lens.map modifies values correctly`` () =
    Lens_map fst_ (fun x -> x + x) ("Good",()) =! ("GoodGood",())

[<Fact>]
let ``test Ismorphism composition over a lens gets value`` () =
    Lens_get (fst_ >-> chars) ("Good",()) =! [| 'G'; 'o'; 'o'; 'd' |]

[<Fact>]
let ``test Ismorphism composition over a lens sets value`` () =
    Lens_set (fst_ >-> chars) [| 'G'; 'o'; 'o'; 'd' |] ("Bad",()) =! ("Good",())

[<Fact>]
let ``test Ismorphism composition over a lens gets value over multiple isomorphisms`` () =
    Lens_get (fst_ >-> chars >-> rev) ("dooG",()) =! [| 'G'; 'o'; 'o'; 'd' |]

[<Fact>]
let ``test Ismorphism composition over a lens sets value over multiple isomorphisms`` () =
    Lens_set (fst_ >-> chars >-> rev) [| 'd'; 'o'; 'o'; 'G' |] ("Bad",()) =! ("Good",())

// Skipped: module-level mutable variables don't work in Erlang without process dictionary
...
// Skipped: depends on module-level mutable variables

[<Fact>]
let ``test Local values returning lambdas work`` () =
    let mutable mutableValue = 0
...
[<Fact>]
let ``test Lambdas can be partially applied`` () =
    partialApplication (+) |> equal 29

[<Fact>]
let ``test Flattened lambdas can be composed`` () = // See #704
    let f = (+) >> id
...
[<Fact>]
let ``test TraitCall can resolve overloads with a single generic argument`` () =
    implicitMethod !+"hello" 5 |> equal 1
    implicitMethod !+6       5 |> equal 2

[<Fact>]
let ``test NestedLambdas`` () =
    let mutable m = 0
    let f i =
        m <- i
        fun j ->
...
[<Fact>]
let ``test More than two lambdas can be nested`` () =
    let mutable mut = 0
    let f x =
        mut <- mut + 1
        fun y z ->
...
[<Fact>]
let ``test Multiple nested lambdas can be partially applied`` () =
    let mutable mut = 0
    let f x y z =
        mut <- mut + 1
        fun u ->
...
[<Fact>]
let ``test No errors because references to missing unit args`` () =
    let foofy str =
        fun () -> "foo" + str
    let f1 = foofy "bar"
    f1 () |> equal "foobar"
...
[<Fact>]
let ``test Aether works II`` () = // See #2101
...
[<Fact>]
let ``test Trait calls work with record fields`` () =
...
[<Fact>]
let ``test Unit expression arguments are not removed`` () =
    let mutable x = 0
    let foo i =
        x <- i
    let doNothing () = ()
...
[<Fact>]
let ``test CurriedLambda don't delay side effects unnecessarily`` () = // See #996
    ...
    let inline applyTup2Inline f1 f2 x =
        ...
        let b = f2 x
        (a,b)
    let _a, _b = applyTup2 id mutateAndLambdify 2685397
    sprintf "%A" mutableValue3 |> equal "2685397"
    let _a2, _b2 = applyTup2Inline id mutateAndLambdify 843252
...
[<Fact>]
let ``test Point-free style with multiple arguments works`` () = // See #1041
    let initialValue = { Types.Email = Types.StringField.Empty }
    let m = State.update "m" initialValue
    m.Email.Value |> equal "m"

[<Fact>]
let ``test Function generic type alias works`` () = // See #1121
    let five = fun _ -> 5
...
[<Fact>]
let ``test Function generic type alias works II`` () =
    let three = fun _ -> 3
...
[<Fact>]
let ``test Piping to an alias of a function which returns a function works`` () = // See #2657
...
[<Fact>]
let ``test Piping to an alias of a function which returns a function works II`` () = // See #2657
    let f a b = sprintf $"{a} {b}"
...
[<Fact>]
let ``test Option.apply (<*>) non-curried`` () =
    let f x = x + 1
    let r = Some f <*> Some 2
    r |> equal (Some 3)

[<Fact>]
let ``test Option.apply (<*>) auto curried`` () =
    let f x y = x + y
...
[<Fact>]
let ``test Option.apply (<*>) manually curried workaround`` () =
    let f x =
        let f' = fun y -> x + y
        f'
    let r = Some f <*> Some 2 <*> Some 3
...
[<Fact>]
let ``test Currying/uncurrying works`` () =
...
// Skipped: module-level mutable counter
...
// Skipped: BaseClass with function constructor params not yet supported
...
[<Fact>]
let ``test SRTP with ActivePattern works`` () =
    (lengthWrapper []) |> equal 0
    (lengthWrapper [1;2;3;4]) |> equal 4
    lengthFixed |> equal 3

[<Fact>]
let ``test Applying to a function returned by a member works`` () =
    equal (1,5) baz
    equal (1,5) baz2

[<Fact>]
let ``test Applying to a function returned by a local function works`` () =
    let foo a b c d = a , b + c d
    let bar a = foo 1 a
    let baz = bar 2 (fun _ -> 3) ()
    equal (1,5) baz

// Skipped: depends on module-level mutable counter
...
[<Fact>]
let ``test Partially applied curried lambdas capture this`` () =
    let foo = Foo3()
...
[<Fact>]
let ``test Curried function options work`` () =
...
[<Fact>]
let ``test Point-free works when passing a 2-arg function`` () =
    Pointfree.y |> equal (Some 3)

[<Fact>]
let ``test Functions in record fields are uncurried`` () =
    ...
    f 4 4 2 |> equal 6
    apply3_ r.myFunction 5 7 4 |> equal 8
    apply (r.myFunction 1 1 |> Some) (Some 5) |> equal (Some -3)

[<Fact>]
let ``test Uncurried functions in record fields can be partially applied`` () =
...
[<Fact>]
let ``test Lambdas with tuple arguments don't conflict with uncurrying`` () = // See #1448
    let revert xs =
...
[<Fact>]
let ``test Sequence of functions is uncurried in folding`` () =
    let vals = [(4,5); (6,7)]
    let ops  = [(+); (-)]
...
[<Fact>]
let ``test failwithf is not compiled as function`` () =
    let makeFn value =
        if value then
            fun x -> x + x
        else
...
// Skipped: PseudoElmish tests depend on module-level mutable state

[<Fact>]
let ``test Fix lambda uncurry/curry semantic #1836`` () =
...
[<Fact>]
let ``test Arguments passed to point-free function are uncurried`` () = // See #1959
...
[<Fact>]
let ``test fold produces incorrect output when state is a function with arity > 1`` () = // See #2035
...
[<Fact>]
let ``test Assigning a function with arity > 1 to a scoped mutable variable #2046`` () =
...
[<Fact>]
let ``test Uncurrying works with generic records returning lambdas`` () =
...
[<Fact>]
let ``test Uncurrying works with generic anonymous records returning lambdas`` () =
    applyFooInAnonRecord {| foo = fun x y -> x ** y |} 5. 2. |> equal 25.
    let f = applyFooInAnonRecord {| foo = fun x y -> x ** y |} 5.
    f 3. |> equal 125.

[<Fact>]
let ``test Curried functions being mangled via DU, List.fold and match combination #2356`` () =
...
[<Fact>]
let ``test Option uncurrying #2116`` () =
...
[<Fact>]
let ``test Functions passed to an object expression are uncurried`` () =
    let mutable d = 0
    let getAdder x =
        d <- d + 1
        fun y z -> x + y + z
...
[<Fact>]
let ``test Iterating list of functions #2047`` () =
    let mutable s = "X"
...
[<Fact>]
let ``test Aliasing a function wrapping a multi-arity function in point-free style #2045`` () =
...
[<Fact>]
let ``test curried function of arity 2 works`` () =
    let res =
...
[<Fact>]
let ``test curried function of arity 8 works`` () =
    let actual =
        let onSubmit =
            fun (arg1 : string)
                (arg2 : bool)
...
[<Fact>]
let ``test curried function of arity 10 works`` () =
    let actual =
        let onSubmit =
            fun (arg1 : string)
                (arg2 : bool)
...
[<Fact>]
let ``test Trait call works with multiple inlined functions I`` () = // See #2809
    let account = Some { MultipleInlines.Details = { Tag = "Test"; InsurancePolicyData = None }}
...
[<Fact>]
let ``test Trait call works with multiple inlined functions II`` () = // See #2809
...
let mutable mutableValue_app = 0

let moduleValueReturnsLambda =
    mutableValue_app <- 5
    fun () -> mutableValue_app * 2

let moduleMethodReturnsLambda i =
    mutableValue_app <- i
    fun j -> mutableValue_app * j

type LambdaFactory() =
    member x.ClassPropertyReturnsLambda =
        mutableValue_app <- 5
        fun i -> mutableValue_app * i
    member x.ClassMethodReturnsLambda y =
        mutableValue_app <- y
        fun z -> mutableValue_app * z

// TODO: Module-level mutable side effects don't execute during Erlang module load
// [<Fact>]
// let ``test Module values/methods returning lambdas work`` () =
...
// TODO: Module-level mutable side effects don't execute during Erlang module load
// [<Fact>]
// let ``test Class properties/methods returning lambdas work`` () =
//     let x = LambdaFactory()
...
let mutable counter_app = 0
let next_app () =
    let result = counter_app
...
let ADD_app = adder_app ()

// TODO: Module-level mutable side effects don't execute during Erlang module load
// [<Fact>]
// let ``test Partially applied functions don't duplicate side effects`` () =
//     let result = ADD_app 1 + ADD_app 2 + ADD_app 3
//     result |> equal 6
//     counter_app |> equal 1

// --- Local inline lambdas ---

[<Fact>]
let ``test Local inline lambdas work standalone`` () =
...
// TODO: Mutual recursive class hierarchy (Node/Element/HTMLElement) generates wrong ctor arity in Beam
// [<Fact>]
// let ``test Closures generated by casts work`` () = ...
