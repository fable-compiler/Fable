...
"""
      let formatted =
        compile source
        |> List.filter (fun log -> log.Code = Some "FABLE0001")
        |> List.map (Fable.Cli.Main.Util.formatLog Compiler.Cached.projDir)

      match formatted with
      | [] -> failwith "Expected a FABLE0001 warning"
      | messages -> equal true (messages |> List.forall (fun m -> m.Contains "warning FABLE FABLE0001:"))
...
open System.Globalization
"abc".StartsWith(
    "a", true, CultureInfo.InvariantCulture) |> ignore // fable-disable-line FABLE0001
"""
      compile source
      |> Assert.Code.noWarning "FABLE0001"
      |> ignore
...
"""
    ...
    testCase "A justification after -- is not parsed as codes" <| fun _ ->
      let source =
        """
open System.Globalization
"abc".StartsWith("a", true, CultureInfo.InvariantCulture) |> ignore // fable-disable-line FABLE0001 -- culture is irrelevant here
"""
      compile source
      |> Assert.Code.noWarning "FABLE0001"
      |> Assert.Code.noWarning "FABLE0003"
      |> ignore
...
"""
      ...
      |> Assert.Code.noWarning "FABLE0004"
      |> ignore
...
let inline startsWithCulture (s: string) =
    s.StartsWith("a", true, CultureInfo.InvariantCulture) // fable-disable-line FABLE0001
...
startsWithCulture "abc" |> ignore // fable-disable-line FABLE0001
"""
      compile source
      |> Assert.Code.warning "FABLE0001"
      |> ignore
