...
module Util =
    ...
    let runFableWithArgs projectDir args =
        run (
            "dotnet run -c Release -p src/Fable.Cli -- "
            + projectDir
            + " "
            + String.concat " " args
        )
...
module Unused =
    ...
    let downloadAndExtractTo (url: string) (targetDir: string) =
        sprintf "npx download --extract --out %s \"%s\"" targetDir url
        |> run
    ...
    let coverage () =
        ...
        if not (pathExists "./bin/tools/reportgenerator")
           && not (pathExists "./bin/tools/reportgenerator.exe") then
...
let buildLibraryWithOptions (opts: {| watch: bool |}) =
    ...
    if opts.watch then
        Async.Parallel [ runNpmScriptAsync "tsc" [ "--project " + projectDir; "--watch" ]
...
let buildLibrary () =
    buildLibraryWithOptions {| watch = false |}

let watchLibrary () =
    buildLibraryWithOptions {| watch = true |}
...
let testJsFast () =
    runFableWithArgs "src/fable-standalone/src" [ "--forcePkgs" ]
...
let buildStandalone (opts: {| minify: bool; watch: bool |}) =
    ...
    runFableWithArgs (projectDir + "/Worker") [ "--outDir " + buildDir + "/worker" ]
    ...
    // print bundle size
    fileSizeInBytes (distDir </> "bundle.min.js")
    / 1000.
    |> printfn "Bundle size: %fKB"

    fileSizeInBytes (distDir </> "worker.min.js")
    / 1000.
...
node --eval "require('ghreleases').create({{
    ...
    body: [{notes}].join('\n'),}}, (err, res) => {{    if (err != null) {{        console.error(err)    }}}})""""
            ...
            with ex -> printfn "Github release failed: %s" ex.Message
        }
        |> runAsyncWorkflow
...
let copyFcsRepo sourceDir =
    ...
    [ "src/fsharp"
      ...
      "src/fsharp/symbols"
      "src/fsharp/utils" ]
    |> List.iter (fun path -> copyDirNonRecursive (sourceDir </> path) (targetDir </> path))
...
let packages =
    [ "Fable.AST", doNothing
      ...
      (fun () ->
          ...
          |> updateVersionInFableTransforms
...
let publishPackages restArgs =
    let packages =
        ...
        | Some pkg ->
            packages
            |> List.filter (fun (name, _) -> name = pkg)
        | None -> packages
...
| "package" :: _ ->
    let pkgInstallCmd =
        buildLocalPackage (resolveDir "temp/pkg")
...
| ("fable-library-ts"
| "library-ts") :: _ -> buildLibraryTs ()
| ("fable-compiler-js"
| "compiler-js") :: _ -> buildCompilerJs (minify)
| ("fable-standalone"
| "standalone") :: _ -> buildStandalone {| minify = minify; watch = false |}
